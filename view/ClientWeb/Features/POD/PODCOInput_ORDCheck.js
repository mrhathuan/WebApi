...
angular.module('myapp').controller('PODCOInput_ORDCheckCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout) {

    Common.Log('PODCOInput_ORDCheckCtrl');
    ...
    $scope.mts_CustomerOptions = {
        dataSource: Common.DataSource.Local({
            ...
            method: _PODCOInput_ORDCheck.URL.Customer_List,
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number' },
    ...
    $scope.mts_VendorOptions = {
        dataSource: Common.DataSource.Local({
            url: Common.Services.url.POD,
            method: _PODCOInput_ORDCheck.URL.Vendor_List,
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number' },
    ...
    $scope.PODCOCheck_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.POD,
            method: _PODCOInput_ORDCheck.URL.Read,
            readparam: function () {
                return {
        ...
        columns: [
            ...
            },
            {
                ...
                title: '{{RS.OPSDITOGroupProduct.InvoiceNote}}', template: '<input type="text"  class="k-textbox "  ng-model="dataItem.InvoiceNote"  ng-keydown="QuickSave($event,PODCOCheck_grid,dataItem)" style="width: 100%"ng-disabled="dataItem.IsClosed"  ></input>',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            ...
            { field: 'RequestDate', width: 100, sortorder: 16, configurable: true, isfunctionalHidden: false, title: 'Ngày gửi', template: "#=RequestDate==null?' ':Common.Date.FromJsonDMYHMS(RequestDate)#", filterable: false },
            { field: 'CustomerCode', width: 100, sortorder: 17, configurable: true, isfunctionalHidden: false, title: 'Mã khách hàng', filterable: { cell: { operator: 'contains', showOperators: false } } },
            { field: 'CustomerName', width: 100, sortorder: 18, configurable: true, isfunctionalHidden: false, title: 'Tên khách hàng', filterable: { cell: { operator: 'contains', showOperators: false } } },
        ...
        if (data.IsInvoice) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.POD,
                        method: _PODCOInput_ORDCheck.URL.Update,
                        data: { 'item': data },
                        success: function (res) {
                            $rootScope.IsLoading = false;
    ...
    $scope.PODInput_ResetClick = function ($event, grid,data) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.POD,
                    method: _PODCOInput_ORDCheck.URL.Reset,
