...
var _CUSSettingOrder = {
    URL: {
        Read: 'CUSSettingOrder_List',
        Get: 'CUSSettingOrder_Get',
        Delete: 'CUSSettingOrder_Delete'
    }
}

angular.module('myapp').controller('CUSSettingOrder_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('CUSSettingOrder_IndexCtrl');
    $rootScope.IsLoading = false;

    $scope.setting_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.CUS,
            method: _CUSSettingOrder.URL.Read,
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.CUSSetting_EditClick = function ($event, data) {
        ...
        $state.go("main.CUSSettingOrder.Detail", { id: data.ID })
    }
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.CUS,
                    method: _CUSSettingOrder.URL.Delete,
                    data: { item: data },
    ...
    $scope.AddNew_Click = function ($event) {
        $event.preventDefault();
        $state.go("main.CUSSettingOrder.Detail", { id: 0 })
    }
