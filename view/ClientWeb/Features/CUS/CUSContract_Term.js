...
var _CUSContract_Term = {
    URL: {
        Get: 'CUSContract_ContractTerm_Get',
        Save: 'CUSContract_ContractTerm_Save',
        Delete: 'CUSContract_ContractTerm_Delete',
        Price_List: 'CUSContract_ContractTerm_Price_List',
        ...
        TermKPITime_List: 'CUSContract_ContractTerm_KPITime_List',
        TermKPITime_Save: 'CUSContract_ContractTerm_KPITime_SaveExpr',
        TermKPITime_NoTInList: 'CUSContract_ContractTerm_KPITime_NotInList',
        TermKPITime_SaveNoiInList: 'CUSContract_ContractTerm_KPITime_SaveNotInList',

        KPI_Check_Expression: 'CUSContract_KPITime_Check_Expression',
        KPI_Check_KPI: 'CUSContract_KPITime_Check_Hit',
        KPIQuantity_Check_Expression: 'CUSContract_KPIQuantity_Check_Expression',
        KPIQuantity_Check_KPI: 'CUSContract_KPIQuantity_Check_Hit',

        TermKPIQuantity_List: 'CUSContract_ContractTerm_KPIQuantity_List',
        TermKPIQuantity_Save: 'CUSContract_ContractTerm_KPIQuantity_SaveExpr',
        TermKPIQuantity_NoTInList: 'CUSContract_ContractTerm_KPIQuantity_NotInList',
        TermKPIQuantity_SaveNoiInList: 'CUSContract_ContractTerm_KPIQuantity_SaveNotInList',
        CUSContract_KPIQuantity_Get: 'CUSContract_KPIQuantity_Get',
    },
    Param: {
        ContractID: -1,
        TermID: -1,
...
angular.module('myapp').controller('CUSContract_TermCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('CUSContract_TermCtrl');
    $rootScope.IsLoading = false;

    _CUSContract_Term.Param = $.extend(true, _CUSContract_Term.Param, $state.params);
    $scope.TabIndex = 0;
    ...
    Common.Services.Call($http, {
        ...
        method: _CUSContract_Term.URL.Get,
        data: { id: _CUSContract_Term.Param.TermID, contractID: _CUSContract_Term.Param.ContractID },
    ...
    $scope.Update_Click = function ($event, vform) {
        ...
        if (vform()) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.CUS,
                method: _CUSContract_Term.URL.Save,
                data: { item: $scope.Item, contractID: _CUSContract_Term.Param.ContractID },
                success: function (res) {
                    debugger
                    $rootScope.IsLoading = false;
                    $rootScope.Message({ Msg: "Đã cập nhật!" });
                    $state.go('main.CUSContract.Term', { TermID: res, ContractID:_CUSContract_Term.Param.ContractID });
                }
            });
        }
    };
    ...
    $scope.Delete_Click = function ($event) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                Common.Services.Call($http, {
                    url: Common.Services.url.CUS,
                    method: _CUSContract_Term.URL.Delete,
                    data: { ID: $scope.Item.ID },
                    success: function (res) {
                        Common.Services.Error(res, function (res) {
                            $rootScope.Message({ Msg: "Đã xóa!" });
                            $state.go('main.CUSCustomer.Contract', { ID: _CUSContract_Term.Param.ContractID, CustomerID:0 });
                        })
                    }
                });
            }
    ...
    Common.Services.Call($http, {
        ...
        method: _CUSContract_Term.URL.Get_Material,
        success: function (res) {
            ...
            _CUSContract_Term.Data.Material = data;
        }
    });
    ...
    $scope.price_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.CUS,
            method: _CUSContract_Term.URL.Price_List,
            readparam: function () {
                return {
                    contractTermID: _CUSContract_Term.Param.TermID
                }
            },
            model: {
                id: 'ID',
        ...
        columns: [
            ...
            },
            {
                ...
                template: '<a href="/" ng-click="Price_Edit_Click($event,dataItem,price_win,price_win_vform)" class="k-button"><i class="fa fa-pencil"></i></a>' +
                    '<a href="/" ng-click="Price_Delete_Click($event,dataItem)" ng-show="!Item.IsClosed" class="k-button"><i class="fa fa-trash"></i></a>' +
        ...
        Common.Services.Call($http, {
            ...
            success: function (res) {
                if (res.CheckPrice.HasNormal && !res.IsAllRouting) {
                    $state.go("main.CUSContract.PriceDINormal", { PriceID: ID, TermID: res.ContractTermID })
                }
                else {
                    if (res.CheckPrice.HasLevel && !res.IsAllRouting)
                    {
                        $state.go("main.CUSContract.PriceDILevel", { PriceID: ID, TermID: res.ContractTermID })
                    }
                    else {
                        $state.go('main.CUSContract.PriceDI', {
                            PriceID: ID,
                            TermID: _CUSContract_Term.Param.TermID
                        });
                    }
                }
    ...
    $scope.getLinkCODetail = function (ID) {
        Common.Services.Call($http, {
            ...
            success: function (res) {
                if (res.CheckPrice.HasNormal && !res.IsAllRouting) {
                    $state.go("main.CUSContract.PriceCONormal", { PriceID: ID, TermID: res.ContractTermID })
                }
                else {
    ...
    $scope.PriceLoadItem = function (id, win, vform) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.CUS,
            method: _CUSContract_Term.URL.Price_Get,
            data: { priceID: id },
            success: function (res) {
                $rootScope.IsLoading = false;
                $scope.ItemPrice = res;
    ...
    $scope.Price_Delete_Click = function ($event, item) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.CUS,
                    method: _CUSContract_Term.URL.Price_Delete,
                    data: { ID: item.ID },
                    success: function (res) {
                        Common.Services.Call($http, {
                            url: Common.Services.url.CUS,
        ...
        if (!flag) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.CUS,
                        method: _CUSContract_Term.URL.Price_Copy,
                        data: { data: data },
                        success: function (res) {
                            $rootScope.IsLoading = false;
                            Common.Services.Error(res, function (res) {
    ...
    $scope.Price_win_Submit_Click = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.CUS,
            method: _CUSContract_Term.URL.Price_Save,
            data: { contractTermID: _CUSContract_Term.Param.TermID, item: $scope.ItemPrice },
            success: function (res) {
                Common.Services.Call($http, {
                    url: Common.Services.url.CUS,
                    method: _CUSContract_Term.URL.Get,
    ...
    //#region KPI Term  
      $scope.kpi_gridOptions = {
         dataSource: Common.DataSource.Grid($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.TermKPITime_List,
              readparam: function () { return { contractTermID: _CUSContract_Term.Param.TermID } },
              model: {
                  id: 'ID',
                  fields: {
      ...
      $scope.KpiNotInList_gridOptions = {
          dataSource: Common.DataSource.Grid($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.TermKPITime_NoTInList,
              readparam: function () { return { contractTermID: _CUSContract_Term.Param.TermID } },
              model: {
                  id: 'ID',
                  fields: {
      ...
      $scope.KPITime_NotInList_Save_Click = function ($event, grid, win) {
          ...
          if (Common.HasValue(lst)) {
              ...
              Common.Services.Call($http, {
                  url: Common.Services.url.CUS,
                  method: _CUSContract_Term.URL.TermKPITime_SaveNoiInList,
                  data: { lst: lst, contractTermID: _CUSContract_Term.Param.TermID },
                  success: function (res) {
      ...
      $scope.KPI_Check_Expression_Click = function ($event) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.KPI_Check_Expression,
              data: {
      ...
      $scope.KPI_Check_KPI_Click = function ($event) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.KPI_Check_KPI,
              data: {
                  item: $scope.KPIItemCheck,
                  lst: data
              },
      ...
      $scope.KPI_Save_Click = function ($event, grid, win) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.TermKPITime_Save,
              data: { item: $scope.KPIItemCheck, contractTermID: _CUSContract_Term.Param.TermID },
              success: function (res) {
    ...
    //#region KPI Quantity  
      $scope.kpiQuantity_gridOptions = {
          dataSource: Common.DataSource.Grid($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.TermKPIQuantity_List,
              readparam: function () { return { contractTermID: _CUSContract_Term.Param.TermID } },
              model: {
                  id: 'ID',
                  fields: {
      ...
      $scope.KpiQuantityNotInList_gridOptions = {
          dataSource: Common.DataSource.Grid($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.TermKPIQuantity_NoTInList,
              readparam: function () { return { contractTermID: _CUSContract_Term.Param.TermID } },
              model: {
                  id: 'ID',
                  fields: {
      ...
      $scope.KPIQuantity_Check_Click = function ($event, item, win) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.CUSContract_KPIQuantity_Get,
              data: { id : item.ID },
              success: function (res) {
                  $scope.dataexpr = res;
                  var data = [];
      ...
      $scope.KPIQuantity_NotInList_Save_Click = function ($event, grid, win) {
          ...
          if (Common.HasValue(lst)) {
              ...
              Common.Services.Call($http, {
                  url: Common.Services.url.CUS,
                  method: _CUSContract_Term.URL.TermKPIQuantity_SaveNoiInList,
                  data: { lst: lst, contractTermID: _CUSContract_Term.Param.TermID },
                  success: function (res) {
      ...
      $scope.KPIQuantity_Check_Expression_Click = function ($event) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.KPIQuantity_Check_Expression,
              data: {
      ...
      $scope.KPIQuantity_Check_KPI_Click = function ($event) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.KPIQuantity_Check_KPI,
              data: {
                  item: $scope.KPIQuantityCheck,
                  lst: $scope.dataexpr,
              },
      ...
      $scope.KPIQuantity_Save_Click = function ($event, grid, win) {
          ...
          Common.Services.Call($http, {
              url: Common.Services.url.CUS,
              method: _CUSContract_Term.URL.TermKPIQuantity_Save,
              data: { item: $scope.KPIQuantityCheck, contractTermID: _CUSContract_Term.Param.TermID },
              success: function (res) {
                  grid.dataSource.read();
                  win.close();
                  $rootScope.IsLoading = false;
