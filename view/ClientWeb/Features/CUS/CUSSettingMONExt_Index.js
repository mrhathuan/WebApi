...
var _CUSSettingMONExt = {
    URL: {
        Read: 'CUSSettingMONExt_List',
        Delete: 'CUSSettingMONExt_Delete'
    }
}

angular.module('myapp').controller('CUSSettingMONExt_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('CUSSettingMONExt_IndexCtrl');
    $rootScope.IsLoading = false;

    $scope.setting_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.CUS,
            method: _CUSSettingMONExt.URL.Read,
            model: {
                id: 'SettingID',
                fields: {
    ...
    $scope.CUSSetting_EditClick = function ($event, data) {
        ...
        $state.go("main.CUSSettingMonitor.Detail", { id: data.SettingID })
    }
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.CUS,
                    method: _CUSSettingMONExt.URL.Delete,
                    data: { item: data },
    ...
    $scope.AddNew_Click = function ($event) {
        $event.preventDefault();
        $state.go("main.CUSSettingMonitor.Detail", { id: 0 })
    }
