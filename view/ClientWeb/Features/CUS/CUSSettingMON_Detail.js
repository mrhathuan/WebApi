...
var _CUSSettingMONDetail = {
    URL: {
        Get: 'CUSSettingMON_Get',
        Save: 'CUSSettingMON_Save',
    },
    Data: {
        DataCustomer: [],
        DataTransport: [
        ...
        GridAllSetting: [
            ...
            { Required: true, Type: 0, Code: "RegNo", Name: "Xe" },
            ...
            { Required: true, Type: 0, Code: "VENLoadCode", Name: "Vendor bốc xếp lên" },
            { Required: true, Type: 0, Code: "VENUnLoadCode", Name: "Vendor bốc xếp xuống" },
        ],
        GridSelected: [
...
angular.module('myapp').controller('CUSSettingMON_DetailCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('CUSSettingMON_DetailCtrl');
    $rootScope.IsLoading = false;

    _CUSSettingMONDetail.Param = $.extend(true, _CUSSettingMONDetail.Param, $state.params);
    ...
    $scope.cboType_Options.dataSource.data(_CUSSettingMONDetail.Data.TypeTransport);
    ...
    // $scope.AllSettingGrid_Options.dataSource.data(_CUSSettingMONDetail.Data.GridAllSetting)
    ...
    $scope.SelectedSettingGrid_Options.dataSource.data(_CUSSettingMONDetail.Data.GridSelected)
    ...
    $scope.Save_Click = function ($event) {
        ...
        if (!Common.HasValue($scope.Item.CustomerID) && $scope.Item.CustomerID <= 0) {
            ...
            if (!errorSortOrder && hasID && hasTon && hasCBM && hasQuantity) {
                Common.Services.Call($http, {
                    url: Common.Services.url.CUS,
                    method: _CUSSettingMONDetail.URL.Save,
                    data: { item: ItemSend, id: _CUSSettingMONDetail.Param.id },
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.CUS,
        method: _CUSSettingMONDetail.URL.Get,
        data: { id: _CUSSettingMONDetail.Param.id },
        success: function (res) {
            $scope.Item = res;
            if (_CUSSettingMONDetail.Param.id <= 0)
                $scope.Item.RowStart = 2;

            var dataAll = [];
            var dataSource = $scope.SelectedSettingGrid.dataSource;

            var dataGridAllSetting = $.extend([], true, _CUSSettingMONDetail.Data.GridAllSetting)
