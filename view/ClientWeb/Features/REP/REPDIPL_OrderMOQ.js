...
var _REPDIPLOrderMOQ = {
    URL: {
        Search: 'REPDIPL_OrderMOQ',
        Template: 'REPDIPL_OrderMOQTemplate',

        Read_Customer: 'REP_Customer_Read',
        Read_GroupOfProduct: 'REP_GroupOfProduct_Read',
        Read_Stock: 'REPDIPL_StockList',

        OrderMOQ_Export: 'REPDIPL_OrderMOQ_Export',
...
angular.module('myapp').controller('REPDIPL_OrderMOQCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPDIPL_OrderMOQCtrl');
    $rootScope.IsLoading = false;

    $scope.Auth = $rootScope.GetAuth();
    $scope.arrayColumn = ['Cost', 'Income', 'IncomeQuantity', 'IncomeUnitPrice', 'CostQuantity', 'CostUnitPrice', 'Note'];
    $scope.arrayColumnRS = ['Chi', 'Thu', 'Số lượng thu ', 'Đơn giá thu', 'Số lượng chi ', 'Đơn giá chi', 'Ghi chú'];
    //0 string, 1 ToNumber3, 2 Datetime, 3 ToMoney
    $scope.arrayColumnType = [3, 3, 3, 3, 1, 3, 0];
    ...
    $scope.mulCustomer_Options = {
        ...
        change: function () {
            ...
            Common.Services.Call($http, {
                ...
                method: _REPDIPLOrderMOQ.URL.Read_GroupOfProduct,
                data: { lstid: lstid },
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.Read_Stock,
                data: { lstid: lstid },
                success: function (res) {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.REP,
        method: _REPDIPLOrderMOQ.URL.Read_Customer,
        data: {},
        success: function (res) {
    ...
    $scope.LoadData = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.Search,
            ...
            success: function (res) {
                $rootScope.IsLoading = false;
                _REPDIPLOrderMOQ.Data._ListData = res.ListData;
                _REPDIPLOrderMOQ.Data._ListColumn = res.ListColumnMOQ;
                _REPDIPLOrderMOQ.Data._ListColumnKey = res.ListColumnKey;
                $scope.InitGrid();
            },
            error: function (res) {
                $rootScope.IsLoading = false;
    ...
    $scope.InitGrid = function () {
        ...
        $scope.fieldGrid = [];
        Common.Data.Each(_REPDIPLOrderMOQ.Data._ListColumnKey, function (group) {
            var array = [];
        ...
        $scope.OrderMOQ_grid.setOptions({
            dataSource: Common.DataSource.Local({
                data: [],
                model: Model,
                pageSize: 20,
        ...
        //})
        var dataGrid = [];
        Common.Data.Each(_REPDIPLOrderMOQ.Data._ListData, function (data) {
            var count = 0;
            Common.Data.Each(_REPDIPLOrderMOQ.Data._ListColumnKey, function (key) {
                var dataGroup;
                var code = key.KeyCode;
                Common.Data.Each(_REPDIPLOrderMOQ.Data._ListColumn, function (group) {
                    if (group.KeyCode == code && group.OrderGroupProductID == data.OrderGroupProductID) {
                        dataGroup = group;
                    }
                });

                if (dataGroup != null) {
                    data[$scope.fieldGrid[count][0]] = (!Common.HasValue(dataGroup.Cost) || dataGroup.Cost == "") ? " " : Common.Number.ToMoney(dataGroup.Cost);
                    data[$scope.fieldGrid[count][1]] = (!Common.HasValue(dataGroup.Income) || dataGroup.Income == "") ? " " : Common.Number.ToMoney(dataGroup.Income);
                    data[$scope.fieldGrid[count][2]] = (!Common.HasValue(dataGroup.IncomeQuantity) || dataGroup.IncomeQuantity == "") ? " " : Common.Number.ToMoney(dataGroup.IncomeQuantity);
                    data[$scope.fieldGrid[count][3]] = (!Common.HasValue(dataGroup.IncomeUnitPrice) || dataGroup.IncomeUnitPrice == "") ? " " : Common.Number.ToMoney(dataGroup.IncomeUnitPrice);
                    data[$scope.fieldGrid[count][4]] = (!Common.HasValue(dataGroup.CostQuantity) || dataGroup.CostQuantity == "") ? " " : Common.Number.ToNumber3(dataGroup.CostQuantity);
                    data[$scope.fieldGrid[count][5]] = (!Common.HasValue(dataGroup.CostUnitPrice) || dataGroup.CostUnitPrice == "") ? " " : Common.Number.ToMoney(dataGroup.CostUnitPrice);
                    data[$scope.fieldGrid[count][6]] = dataGroup.Note;
                } else {
        ...
        $scope.OrderMOQ_grid.dataSource.data(dataGrid);
        $rootScope.IsLoading = false;
    }
    ...
    $scope.Excel_Click = function ($event) {
        ...
        var request = Common.Request.CreateFromGrid($scope.OrderMOQ_grid.dataSource);
        ...
        $rootScope.UploadFile({
            ...
            Complete: function (file) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIPLOrderMOQ.URL.Template,
    ...
    $scope.SettingReport = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.SettingList,
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIPLOrderMOQ.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIPLOrderMOQ.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.SettingReport_ActionClick = function ($event, data) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.SettingAction,
    ...
    $scope.customerNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.SettingCusNotInList,
    ...
    $scope.customerNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.SettingCusNotInSave,
    ...
    $scope.customer_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.SettingCusDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListCustomer)) {
    ...
    $scope.gopNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.SettingGOPNotInList,
    ...
    $scope.gopNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.SettingGOPNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.gop_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.SettingGOPDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.stockNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.SettingStockNotInList,
    ...
    $scope.stockNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.SettingStockNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
    ...
    $scope.stock_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPLOrderMOQ.URL.SettingStockDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListStock)) {
    ...
    $scope.REPDIPL_OrderMOQ_Export = function ($event) {
        $event.preventDefault();

        $rootScope.IsLoading = true;
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPLOrderMOQ.URL.OrderMOQ_Export,
            data: { lstid: $scope.Item.lstCustomerID, lstgroupid: $scope.Item.lstGroupID, lststockid: $scope.Item.lstStockID, dtfrom: $scope.Item.DateFrom, dtto: $scope.Item.DateTo },
            success: function (res) {
                $rootScope.IsLoading = false;
                $rootScope.DownloadFile(res);
