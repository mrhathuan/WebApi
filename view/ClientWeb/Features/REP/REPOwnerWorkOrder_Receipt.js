...
var _REPOwnerWorkOrder_Receipt = {
    URL: {
        Search: 'REPOwner_Receipt',
        Template: 'REPOwnerWorkOrder_ReceiptTemplate',

        Receipt_Export: 'REPOwner_Receipt_Export',
...
angular.module('myapp').controller('REPOwnerWorkOrder_ReceiptCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPOwnerWorkOrder_ReceiptCtrl');
    $rootScope.IsLoading = false;

    $scope.Auth = $rootScope.GetAuth();
    $scope.SettingItem = { ID: 0 };
    $scope.arrayColumn = ['Quantity', 'UnitPrice', 'Total', 'Note'];
    $scope.arrayColumnRS = ['Số lượng', 'Đơn giá', 'Thành tiền', 'Ghi chú'];
    ...
    $scope.SearchData = function () {
        ...
        Common.Services.Call($http, {
            ...
            method: _REPOwnerWorkOrder_Receipt.URL.Search,
            data: { dtfrom: $scope.Item.DateFrom, dtto: $scope.Item.DateTo },
            success: function (res) {
                angular.forEach(res, function (item, dix) {
                    if (Common.HasValue(item)) {
                        if (Common.HasValue(item.Date))
                            item.Date = kendo.parseDate(item.Date);
                    }
                });
                _REPOwnerWorkOrder_Receipt.Data._listData = res.ListData;
                _REPOwnerWorkOrder_Receipt.Data._listColumn = res.ListColumn;
                _REPOwnerWorkOrder_Receipt.Data._listColumnKey = res.ListColumnKey;
    ...
    $scope.InitGrid = function () {
        ...
        var Model = {
            id: 'ReceiptID',
            fields: {
                ReceiptID: { type: 'number', editable: false },
                Date: { type: 'date' },
            }
        }
        var GridColumn = [
        ...
        $scope.fieldGrid = [];
        Common.Data.Each(_REPOwnerWorkOrder_Receipt.Data._listColumnKey, function (group) {
            var array = [];
        ...
        $scope.rep_grid.setOptions({
            ...
            height: '100%', pageable: true, sortable: true, columnMenu: false, resizable: true, reorderable: false, filterable: { mode: 'row' }, editable: { mode: 'incell' }, pageSize: 20,
            columns: GridColumn
        })

        var dataGrid = [];
        Common.Data.Each(_REPOwnerWorkOrder_Receipt.Data._listData, function (data) {
            for (var i = 0 ; i < _REPOwnerWorkOrder_Receipt.Data._listColumn.length ; i++) {
                var group = _REPOwnerWorkOrder_Receipt.Data._listColumn[i];
                if (group.ReceiptID == data.ReceiptID) {
                    var count = 0;
                    Common.Data.Each(_REPOwnerWorkOrder_Receipt.Data._listColumnKey, function (key) {
                        var dataGroup = null;
                        if (group.MaterialID == key.MaterialID) {
                            dataGroup = group;
                            _REPOwnerWorkOrder_Receipt.Data._listColumn.splice(i, 1);
                        }

                        if (dataGroup != null) {
                            data[$scope.fieldGrid[count][0]] = dataGroup.Quantity;
                            data[$scope.fieldGrid[count][1]] = dataGroup.UnitPrice;
                            data[$scope.fieldGrid[count][2]] = dataGroup.Total;
                            data[$scope.fieldGrid[count][3]] = dataGroup.Note;
                        } else {
    ...
    $scope.Excel_Click = function ($event) {
        ...
        $rootScope.UploadFile({
            ...
            Complete: function (file) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_Receipt.URL.Template,
    ...
    $scope.SettingReport = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_Receipt.URL.SettingList,
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_Receipt.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_Receipt.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.customerNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_Receipt.URL.SettingCusNotInList,
    ...
    $scope.customerNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_Receipt.URL.SettingCusNotInSave,
    ...
    $scope.customer_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_Receipt.URL.SettingCusDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListCustomer)) {
    ...
    $scope.gopNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_Receipt.URL.SettingGOPNotInList,
    ...
    $scope.gopNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_Receipt.URL.SettingGOPNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.gop_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_Receipt.URL.SettingGOPDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.Excel_Export = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_Receipt.URL.Receipt_Export,
            data: { dtfrom: $scope.Item.DateFrom, dtto: $scope.Item.DateTo },
            success: function (res) {
                $rootScope.IsLoading = false;
                $rootScope.DownloadFile(res);
