...
angular.module('myapp').controller('REPDIPL_DetailCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPDIPL_DetailCtrl');
    ...
    $scope.mulCustomer_Options = {
        ...
        change: function () {
            ...
            Common.Services.Call($http, {
                ...
                method: _REPDIPL_Detail.URL.Read_GroupOfProduct,
                data: { lstid: lstid },
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPL_Detail.URL.Read_Stock,
                data: { lstid: lstid },
                success: function (res) {
    ...
    $scope.Search_Click = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPL_Detail.URL.Search,
    ...
    //            Common.Services.Call($http, {
    //                url: Common.Services.url.REP,
    //                method: _REPDIPL_Detail.URL.Template,
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIPL_Detail.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIPL_Detail.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.SettingReport_ActionClick = function ($event, data) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPL_Detail.URL.SettingAction,
    ...
    $scope.gopNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPDIPL_Detail.URL.SettingGOPNotInList,
    ...
    $scope.gopNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPL_Detail.URL.SettingGOPNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.gop_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPL_Detail.URL.SettingGOPDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.stockNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPDIPL_Detail.URL.SettingStockNotInList,
    ...
    $scope.stockNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPL_Detail.URL.SettingStockNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
    ...
    $scope.stock_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIPL_Detail.URL.SettingStockDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListStock)) {
    ...
    $scope.REPDIPL_Detail_Export = function ($event) {
        $event.preventDefault();

        $rootScope.IsLoading = true;
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPDIPL_Detail.URL.Detail_Export,
            data: {},
            success: function (res) {
                $rootScope.IsLoading = false;
                $rootScope.DownloadFile(res);
