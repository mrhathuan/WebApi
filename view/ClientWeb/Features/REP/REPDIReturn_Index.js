...
var _REPDIReturn_Index = {
    URL: {
        ...
        SettingAction: 'REPDIReturn_SettingDownload',
    },
    Param: {
    },
}

angular.module('myapp').controller('REPDIReturn_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPDIReturn_IndexCtrl');
    ...
    Common.Services.Call($http, {
        ...
        method: _REPDIReturn_Index.URL.SettingList,
    ...
    $scope.LoadSettingItem = function (win, data, vform) {
        if (data != null) {
            ...
            $scope.SettingItem.TypeDateRange = 3;
        }
        vform({ clear: true });
        win.center().open();
    }
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIReturn_Index.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPDIReturn_Index.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.cboTypeExport_options = {
        ...
        dataSource: [
            { ID: 1, ValueName: 'Công nợ trả về chi tiết' },
        ],
        change: function (e) { }
    }
    ...
    $scope.SettingReport_ActionClickConfirm = function ($event, vform, win) {
        ...
        if (vform() && !error) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIReturn_Index.URL.SettingAction,
    ...
    $scope.customerNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPDIReturn_Index.URL.SettingCusNotInList,
    ...
    $scope.customerNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIReturn_Index.URL.SettingCusNotInSave,
    ...
    $scope.customer_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPDIReturn_Index.URL.SettingCusDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListCustomer)) {
