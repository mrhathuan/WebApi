...
var _REPOwnerPLVehicle_Detail = {
    URL: {
        Search: 'REPOwner_VehicleFee',
        Template: 'REPOwner_VehicleFee_Template',

        Excel_Export: 'REPOwner_VehicleFee_Detail_Export',
...
angular.module('myapp').controller('REPOwnerPLVehicle_DetailCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPOwnerPLVehicle_DetailCtrl');
    ...
    $scope.Search_Click = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerPLVehicle_Detail.URL.Search,
    ...
    $scope.Excel_Click = function ($event) {
        ...
        $rootScope.UploadFile({
            ...
            Complete: function (file) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerPLVehicle_Detail.URL.Template,
    ...
    $scope.SettingReport = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerPLVehicle_Detail.URL.SettingList,
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerPLVehicle_Detail.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerPLVehicle_Detail.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.customerNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerPLVehicle_Detail.URL.SettingCusNotInList,
    ...
    $scope.customerNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerPLVehicle_Detail.URL.SettingCusNotInSave,
    ...
    $scope.customer_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerPLVehicle_Detail.URL.SettingCusDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListCustomer)) {
    ...
    $scope.gopNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerPLVehicle_Detail.URL.SettingGOPNotInList,
    ...
    $scope.gopNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerPLVehicle_Detail.URL.SettingGOPNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.gop_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerPLVehicle_Detail.URL.SettingGOPDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.Excel_Export = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerPLVehicle_Detail.URL.Excel_Export,
            data: { },
            success: function (res) {
                $rootScope.IsLoading = false;
                $rootScope.DownloadFile(res);
