...
var _REPOwnerWorkOrder_Index = {
    ...
    Data: {
        TypeOfReceipt: [
            { ID: 1, ValueName: 'Phiếu vật tư, nhiên liệu' },
            { ID: 2, ValueName: 'Phiếu sửa chữa nhỏ' },
        ],
        TypeExport: [
            [
...
angular.module('myapp').controller('REPOwnerWorkOrder_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPOwnerWorkOrder_IndexCtrl');
    ...
    Common.Services.Call($http, {
        ...
        method: _REPOwnerWorkOrder_Index.URL.SettingList,
    ...
    $scope.LoadSettingItem = function (win, data, vform) {
        if (data != null) {
            ...
            $scope.SettingItem = { ID: 0 };
            $scope.SettingItem.TypeOfReceipt = 1;
        ...
        var typeExportID = $scope.SettingItem.TypeExport;

        $timeout(function () {
            $scope.cboTypeExport.setDataSource(_REPOwnerWorkOrder_Index.Data.TypeExport[$scope.SettingItem.TypeOfReceipt - 1]);
            $scope.SettingItem.TypeExport = typeExportID;
        }, 10);
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_Index.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_Index.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.cboTypeOfReceipt_options = {
        autoBind: true, valuePrimitive: true, ignoreCase: true, filter: "contains",
        suggest: true, dataTextField: 'ValueName', dataValueField: 'ID',
        dataSource: _REPOwnerWorkOrder_Index.Data.TypeOfReceipt,
        change: function (e) {
            $timeout(function () {
                $scope.cboTypeExport.setDataSource(_REPOwnerWorkOrder_Index.Data.TypeExport[$scope.SettingItem.TypeOfReceipt - 1]);
                $scope.SettingItem.TypeExport = 1;
