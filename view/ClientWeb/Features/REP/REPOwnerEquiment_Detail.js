...
var _REPOwnerEquiment_Detail = {
    URL: {
        Search: 'REPOwner_Equipment',

        Detail_Export: 'REPOwnerEquiment_Detail_Export',
...
angular.module('myapp').controller('REPOwnerEquiment_DetailCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPOwnerEquiment_DetailCtrl');
    ...
    $scope.REPOwnerEquiment_gridOptions = {
        dataSource: Common.DataSource.Local({
            data: [],
            model: {
                id: 'CostID',
                fields: {
                    AssetID: { type: 'number', editable: false },
                    Date: { type: 'date' },
                }
            },
            pageSize: 20
        ...
        columns: [
            ...
            {
                field: 'Name', title: '<b>Thiết bị</b><br>[Name]', width: '120px',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
            ...
            {
                field: 'BaseValue', title: '<b>Giá gốc</b><br>[BaseValue]', width: '120px', template: '#=BaseValue==null?" ":Common.Number.ToMoney(BaseValue)#',
                filterable: { cell: { operator: 'gte', showOperators: false } }
            },
            {
                field: 'CurrentValue', title: '<b>Giá hiện tại</b><br>[CurrentValue]', width: '120px', template: '#=CurrentValue==null?" ":Common.Number.ToMoney(CurrentValue)#',
                filterable: { cell: { operator: 'gte', showOperators: false } }
            },
            {
            ...
            {
                field: 'CurrentStatus', title: '<b>Tình trạng</b><br>[CurrentStatus]', width: '120px',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
        ...
        $('.dtp-filter-from').kendoDatePicker({
            ...
            change: function (e) {
                try {
                    ...
                    var filters = [];
                    if (Common.HasValue($scope.REPOwnerEquiment_gridOptions.dataSource.filter())) {
                        filters = $scope.REPOwnerEquiment_gridOptions.dataSource.filter().filters;
                    ...
                    $scope.REPOwnerEquiment_gridOptions.dataSource.filter(filters);
                }
                catch (e) {
    ...
    $scope.Search_Click = function ($event) {
        ...
        Common.Services.Call($http, {
            ...
            method: _REPOwnerEquiment_Detail.URL.Search,
            ...
            success: function (res) {
                ...
                $scope.REPOwnerEquiment_gridOptions.dataSource.data(res);
            },
            error: function (res) {
                $rootScope.IsLoading = false;
            }
    ...
    $scope.SettingReport = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerEquiment_Detail.URL.SettingList,
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerEquiment_Detail.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerEquiment_Detail.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.SettingReport_ActionClick = function ($event, data) {
        ...
        var request = Common.Request.CreateFromGrid($scope.REPOwnerEquiment_gridOptions.dataSource);
        $rootScope.IsLoading = true;
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
    ...
    $scope.Excel_Export = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerEquiment_Detail.URL.Detail_Export,
            data: {},
            success: function (res) {
                $rootScope.IsLoading = false;
                $rootScope.DownloadFile(res);
