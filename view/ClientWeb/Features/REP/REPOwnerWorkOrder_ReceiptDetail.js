...
var _REPOwnerWorkOrder_ReceiptDetail = {
    URL: {
        Search: 'REPOwner_ReceiptDetail',
        Template: 'REPOwnerWorkOrder_ReceiptDetailTemplate',

        ReceiptDetail_Export: 'REPOwnerWorkOrder_ReceiptDetail_Export',
...
angular.module('myapp').controller('REPOwnerWorkOrder_ReceiptDetailCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('REPOwnerWorkOrder_ReceiptDetailCtrl');
    ...
    $scope.REPOwnerWorkOrder_gridOptions = {
        ...
        columns: [
            ...
            {
                field: 'MaterialName', title: '<b>Nhiên liệu</b><br>[MaterialCode]', width: '120px',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
    ...
    $scope.Search_Click = function ($event) {
        ...
        Common.Services.Call($http, {
            ...
            method: _REPOwnerWorkOrder_ReceiptDetail.URL.Search,
    ...
    //            Common.Services.Call($http, {
    //                url: Common.Services.url.REP,
    //                method: _REPOwnerWorkOrder_ReceiptDetail.URL.Template,
    ...
    $scope.SettingReport = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingList,
    ...
    $scope.SettingReport_SaveClick = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            if (!error) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingSave,
    ...
    $scope.SettingDelete = function (win, datasend) {
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.REP,
                    method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingDelete,
                    data: { lst: datasend },
                    success: function (res) {
                        $rootScope.Message({
                            Type: Common.Message.Type.Notify,
    ...
    $scope.customerNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingCusNotInList,
    ...
    $scope.customerNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingCusNotInSave,
    ...
    $scope.customer_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingCusDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListCustomer)) {
    ...
    $scope.gopNotIn_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingGOPNotInList,
    ...
    $scope.gopNotIn_Save_Click = function ($event, win, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingGOPNotInSave,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.gop_Delete = function ($event, grid) {
        ...
        if (datasend.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.REP,
                method: _REPOwnerWorkOrder_ReceiptDetail.URL.SettingGOPDeleteList,
                data: { item: $scope.SettingItem, lst: datasend },
                success: function (res) {
                    $scope.SettingItem = res;
                    if (Common.HasValue(res.ListGroupProduct)) {
    ...
    $scope.Excel_Export = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.REP,
            method: _REPOwnerWorkOrder_ReceiptDetail.URL.ReceiptDetail_Export,
            data: {},
            success: function (res) {
                $rootScope.IsLoading = false;
                $rootScope.DownloadFile(res);
