...
var _FLMAsset_Tractor_Edit = {
    URL: {
        Get_Tractor: 'FLMAsset_Tractor_Get',
        Save_Tractor: 'FLMAsset_Tractor_Update',
        Delete_Tractor: 'FLMAsset_Tractor_Destroy',
        ...
        Get_TractorDriverList: 'FLMAsset_Tractor_Read',
        Get_RentList: 'FLMAsset_RentRead',
        Get_RomoocList: 'FLMAsset_Tractor_GetRomooc',
...
angular.module('myapp').controller('FLMAsset_EditTractorCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', 'openMap', function ($rootScope, $scope, $http, $location, $state, $timeout, openMap) {
    Common.Log('FLMAsset_EditTractorCtrl');
    ...
    _FLMAsset_Tractor_Edit.Param = $.extend(true, _FLMAsset_Tractor_Edit.Param, $state.params);
    $scope.TractorItem = null;
    ...
    if (_FLMAsset_Tractor_Edit.Param.AssetID > 0) { $scope.IsShowTab = true; $scope.IsNewVehicle = false; }

    $scope.TabIndex = 1;
    $scope.FLMAsset_EditTractor_TabOptions = {
    ...
    Common.Services.Call($http, {
        ...
        method: _FLMAsset_Tractor_Edit.URL.Get_Tractor,
        data: { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID },
        success: function (res) {
            $scope.TractorItem = res;
            var flag = false;
            $.each($scope.dataExprInputDay, function (i, v) {
                if (v.value == $scope.TractorItem.ExprInputDay) {
                    flag = true;
                }
            });

            if (!flag) {
                $scope.dataExprInputDay[0].value = $scope.TractorItem.ExprInputDay;
            }
            ...
            if (Common.HasValue(res.ExprTOMaster) && res.ExprTOMaster != '')
    ...
    $scope.FLMAsset_Tractor_SaveClick = function ($event, vform) {
        $event.preventDefault();
        if (vform()) {
            $rootScope.IsLoading = true;
            Common.Services.Call($http, {
                url: Common.Services.url.FLM,
                method: _FLMAsset_Tractor_Edit.URL.Save_Tractor,
                data: { item: $scope.TractorItem },
                success: function (res) {
                    $rootScope.IsLoading = false;
                    $rootScope.Message({ Msg: 'Đã lưu', Type: Common.Message.Type.Notify, NotifyType: Common.Message.NotifyType.SUCCESS });
                    $state.go('main.FLMAsset.EditTractor', { AssetID: res });
                },
                error: function (e) {
                    $rootScope.IsLoading = false;
                }
    ...
    $scope.FLMAsset_Tractor_DelClick = function ($event) {
        $event.preventDefault();
        $rootScope.Message({
            ...
            Ok: function (e) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.FLM,
                    method: _FLMAsset_Tractor_Edit.URL.Delete_Tractor,
                    data: { item: $scope.TractorItem },
                    success: function (res) {
    ...
    $scope.cboRomooc_options = {
        autoBind: true, valuePrimitive: true, ignoreCase: true, filter: "contains",
        suggest: true, dataTextField: 'RegNo', dataValueField: 'RomoocID',
        dataSource: Common.DataSource.Local([], {
            id: 'RomoocID',
            fields: {
                RegNo: { type: 'string' },
        ...
        change: function (e) {
            if (e.sender.selectedIndex >= 0) {
                $scope.IsEdited = true;
                $scope.TractorItem.MaxWeightCal = e.sender.dataItem(e.item).MaxWeight + $scope.TractorItem.MaxWeight;
            }
        }
    }
    Common.Services.Call($http, {
        url: Common.Services.url.FLM,
        method: _FLMAsset_Tractor_Edit.URL.Get_RomoocList,
        data: {},
        success: function (res) {
            $scope.cboRomooc_options.dataSource.data(res.Data)
        }
    });
    ...
    $scope.numMaxWeight_options = {
        format: 'n3', spinners: false, culture: 'en-US', min: 0, step: 1, decimals: 3,
        change: function (e) {
            var item = $scope.cboRomooc.dataItem($scope.cboRomooc.select())
            if (Common.HasValue(item))
                if (item.MaxWeight != '' && item.MaxWeight != null)
                    $scope.TractorItem.MaxWeightCal = item.MaxWeight + this.value();
                else $scope.TractorItem.MaxWeightCal = this.value();
        }
    }
    ...
    $scope.numRemainValue_options = {
        ...
        change: function (e) {
            if ($scope.TractorItem.RemainValue > 0)
                $scope.TractorItem.CurrentValue = $scope.TractorItem.BaseValue - $scope.TractorItem.RemainValue;
            else
                $scope.TractorItem.CurrentValue = $scope.TractorItem.BaseValue;
        }
    }
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.FLM,
        method: _FLMAsset_Tractor_Edit.URL.Get_ALLDriverList,
        data: {},
        success: function (res) {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.FLM,
        method: _FLMAsset_Tractor_Edit.URL.Get_RentList,
        data: {},
        success: function (res) {
    ...
    $scope.LocationAddress_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.LocationAddress_List,
            pageSize: 50,
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.LoadItem = function (win, id) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Location_Get,
            data: { 'ID': id },
            success: function (res) {
    ...
    Common.ALL.Get($http, {
        ...
        success: function (data) {
            _FLMAsset_Tractor_Edit.Data.Country = data;
    ...
    Common.ALL.Get($http, {
        ...
        success: function (data) {
            _FLMAsset_Tractor_Edit.Data.Province = {};
            angular.forEach(data, function (obj, indx) {
                if (Common.HasValue(_FLMAsset_Tractor_Edit.Data.Province[obj.CountryID]))
                    _FLMAsset_Tractor_Edit.Data.Province[obj.CountryID].push(obj);
                else _FLMAsset_Tractor_Edit.Data.Province[obj.CountryID] = [obj];
            })
        }
    })
    ...
    Common.ALL.Get($http, {
        ...
        success: function (data) {
            _FLMAsset_Tractor_Edit.Data.District = {};
            angular.forEach(data, function (obj, indx) {
                if (Common.HasValue(_FLMAsset_Tractor_Edit.Data.District[obj.ProvinceID]))
                    _FLMAsset_Tractor_Edit.Data.District[obj.ProvinceID].push(obj);
                else _FLMAsset_Tractor_Edit.Data.District[obj.ProvinceID] = [obj];
            })
        }
    });
    ...
    $scope.LoadRegionData = function (item) {
        ...
        try {
            ...
            var data = _FLMAsset_Tractor_Edit.Data.Province[countryID];
            ...
            data = _FLMAsset_Tractor_Edit.Data.District[provinceID];
            ...
            if (Common.HasValue($scope.LocationItem.CountryID) && $scope.LocationItem.CountryID > 0) {
                if (Common.HasValue($scope.LocationItem.ProvinceID) && $scope.LocationItem.ProvinceID > 0) {
                    if (Common.HasValue($scope.LocationItem.DistrictID) && $scope.LocationItem.DistrictID > 0) {
                        Common.Services.Call($http, {
                            url: Common.Services.url.FLM,
                            method: _FLMAsset_Tractor_Edit.URL.Location_Save,
    ...
    $scope.LocationDestroy_Click = function ($event, data) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                Common.Services.Call($http, {
                    url: Common.Services.url.FLM,
                    method: _FLMAsset_Tractor_Edit.URL.Location_Delete,
                    data: { 'item': data },
                    success: function (res) {
                        $scope.LocationAddress_grid.dataSource.read();
                        $rootScope.Message({ Msg: 'Thành công', NotifyType: Common.Message.NotifyType.SUCCESS })
    ...
    $scope.LocationAddress_Win_SaveClick = function ($event, win, grid) {
        ...
        if (Common.HasValue(data)) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.FLM,
                method: _FLMAsset_Tractor_Edit.URL.FLM_LocationNotIn_Save,
                data: { locationID: data.ID, assetID: $scope.TractorItem.ID },
                success: function (res) {
                    $rootScope.IsLoading = false;
                    $scope.LocationAddress_gridOptions.dataSource.read();
                    $scope.TractorItem.LocationAddress = data.Address;
                    win.close();
                    $rootScope.Message({ Msg: 'Thành công', NotifyType: Common.Message.NotifyType.SUCCESS });
                },
                error: function (res) {
            ...
            $rootScope.Message({
                ...
                Close: function (e) {
                    Common.Services.Call($http, {
                        url: Common.Services.url.FLM,
                        method: _FLMAsset_Tractor_Edit.URL.Get_Tractor,
                        data: { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID, RegNo: _FLMAsset_Tractor_Edit.Param.RegNo },
                        success: function (res) {
                            $scope.IsEdited = false;
                            $scope.TractorItem = res;
    ...
    $scope.FixedCost_Delete = function ($event, grid) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.FLM,
                    method: _FLMAsset_Tractor_Edit.URL.AssetDelete,
                    data: { assetID: _FLMAsset_Tractor_Edit.Param.AssetID },
                    success: function (res) {
                        Common.Services.Error(res, function () {
                            ...
                            Common.Services.Call($http, {
                                url: Common.Services.url.FLM,
                                method: _FLMAsset_Tractor_Edit.URL.FixedCost_ByAssetList,
                                data: { assetID: _FLMAsset_Tractor_Edit.Param.AssetID },
                                success: function (res) {
    ...
    $scope.FixedCost_Generate = function ($event, grid) {
        ...
        $event.preventDefault();
        var error = false;
        if ($scope.TractorItem.CurrentValue < 0 || !Common.HasValue($scope.TractorItem.CurrentValue)) {
        ...
        if ($scope.TractorItem.DepreciationPeriod <= 0 || !Common.HasValue($scope.TractorItem.DepreciationPeriod)) {
        ...
        if (!Common.HasValue($scope.TractorItem.DepreciationStart)) {
        ...
        if (!error) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.FLM,
                method: _FLMAsset_Tractor_Edit.URL.FixedCost_Generate,
                data: { assetID: _FLMAsset_Tractor_Edit.Param.AssetID },
                success: function (res) {
    ...
    $scope.FixedCost_Save = function ($event, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.FixedCost_Save,
            data: { assetID: _FLMAsset_Tractor_Edit.Param.AssetID, lst: lst },
            success: function (res) {
                $rootScope.Message({ Msg: 'Thành công', NotifyType: Common.Message.NotifyType.SUCCESS });
                Common.Services.Call($http, {
                    url: Common.Services.url.FLM,
                    ...
                    success: function (res) {
                        $scope.depreciation_GridOptions.dataSource.data(res);
                        $scope.TractorItem.HasDepreciation = true;
                        $rootScope.IsLoading = false;
                    },
                    error: function (e) {
                        $rootScope.IsLoading = false;
    ...
    $scope.ChooseExprDay_Click = function ($event, win, check) {
        $event.preventDefault();
        $scope.ExprDay.Exp = $scope.TractorItem.ExprInputDay;
        win.center().open();
    };
    ...
    $scope.ExprDay_SaveClick = function ($event, win) {
        ...
        $.each($scope.dataExprInputDay, function (i, v) {
            if (v.value == $scope.ExprDay.Exp) {
                $scope.TractorItem.ExprInputDay = $scope.ExprDay.Exp;
                flag = true;
            }
        });
        if (!flag) {
    ...
    $scope.ExprDay_CheckClick = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Asset_CheckExpr,
            data: { sExpression: $scope.ExprDay.Exp, assetID: _FLMAsset_Tractor_Edit.Param.AssetID },
            success: function (res) {
                if (res == true) {
    ...
    $scope.currEQM_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Get_EQMList,
            readparam: function () { return { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID } },
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number', editable: false, nullable: true },
    ...
    $scope.ConsumptionGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Read_Consumption,
            readparam: function () { return { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID } },
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.ConsumptionNotChoose_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Read_ConsumptionNotIn,
            readparam: function () { return { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID } },
    ...
    $scope.FLMAsset_ConsumptionGrid_Edit = function ($event, grid) {
        ...
        _FLMAsset_Tractor_Edit.Data.ItemBackup = $.extend(true, {}, $scope.ConsumptionEdit);
        grid.editRow(tr);
    ...
    $scope.FLMAsset_ConsumptionGrid_Save = function ($event, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Save_Consumption,
            data: { item: $scope.ConsumptionEdit, AssetID: _FLMAsset_Tractor_Edit.Param.AssetID },
            success: function (res) {
                $rootScope.IsLoading = false;
    ...
    $scope.FLMAsset_ConsumptionGrid_Delete = function ($event, grid) {
        ...
        if (Common.HasValue(obj)) {
            Common.Services.Call($http, {
                url: Common.Services.url.FLM,
                method: _FLMAsset_Tractor_Edit.URL.Delete_Consumption,
                data: { item: obj },
                success: function (res) {
                    $scope.ConsumptionGrid_Options.dataSource.read()
                    $rootScope.IsLoading = false;
    ...
    $scope.FLMAsset_ConsumptionGrid_Cancel = function ($event, grid) {
        ...
        while (i < data.length) {
            var obj = data[i];
            if (obj.ID == _FLMAsset_Tractor_Edit.Data.ItemBackup.ID) {
                data.splice(i, 1);
                data.splice(i, 0, $.extend(true, {}, _FLMAsset_Tractor_Edit.Data.ItemBackup))
            }
            i++;
        }
        $rootScope.IsLoading = false;
    ...
    $scope.ConsumptionNotChoose_Win_SaveChooseClick = function ($event, win, grid) {
        ...
        var checkErrorKMStart = $.grep(dataChoose, function (o) { return o.KMStart < 0 });
        var checkErrorKMCurrent = $.grep(dataChoose, function (o) { return o.KMCurrent < 0 });
        var checkErrorKMWarning = $.grep(dataChoose, function (o) { return o.KMWarning < 0 });
            ...
            if (!error) {
                $rootScope.Message({
                    ...
                    Ok: function (e) {
                        Common.Services.Call($http, {
                            url: Common.Services.url.FLM,
                            method: _FLMAsset_Tractor_Edit.URL.Save_ConsumptionNotIn,
                            data: { lst: dataChoose, AssetID: _FLMAsset_Tractor_Edit.Param.AssetID },
                            success: function (res) {
                                win.close();
                                $scope.ConsumptionGrid_Options.dataSource.read();
                            }
    ...
    $scope.DepreciationGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.History_Depreciation,
            readparam: function () { return { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID } },
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.FuelHistoryGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.Get_FuelHistory,
            readparam: function () { return { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID } },
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.FeeDefault_GridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.FeeDefault_List,
            readparam: function () { return { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID } },
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.FeeDefault_Delete_Click = function ($event, data) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.FLM,
                    method: _FLMAsset_Tractor_Edit.URL.FeeDefault_Delete,
                    data: { ID: data.ID },
                    success: function (res) {
    ...
    $scope.LoadData_FeeDefault = function (ID, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FLM,
            method: _FLMAsset_Tractor_Edit.URL.FeeDefault_Get,
            data: { ID: ID },
            success: function (res) {
                if (Common.HasValue(res)) {
    ...
    $scope.assetFee_Save_Click = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.FLM,
                method: _FLMAsset_Tractor_Edit.URL.FeeDefault_Save,
                data: { AssetID: _FLMAsset_Tractor_Edit.Param.AssetID, item: $scope.ItemAssetFee },
                success: function (res) {
                    if (Common.HasValue(res)) {
                        $rootScope.Message({
                            Msg: 'Đã cập nhật.',
    ...
    $scope.FLMAsset_Tractor_BackClick = function ($event) {
        $event.preventDefault();
        $state.go("main.FLMAsset.Index")
    }
