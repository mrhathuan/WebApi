...
//#region Data
var _FINFreightAuditVendor = {
    URL: {
        Read: "FINFreightAudit_List",
        Detail: "FINFreightAudit_DetailList",
        Reject: "FINFreightAudit_Reject",
        Accept: "FINFreightAudit_Accept",
        Approved: "FINFreightAudit_Approved",
        Waiting: "FINFreightAudit_Waiting",
        StatusList: "FINFreightAudit_StatusList",
        FreightAuditExport: 'FIN_FreightAudit_Export',
    },
    Data: {
        CookieSearch: "FINFreightAuditVendor_CookieSearch",
        _dataStatus: [],
        _defStatusID: -1,
        _approvedStatusID: -1,
        _waitingStatusID: -1,
...
angular.module('myapp').controller('FINFreightAuditVendor_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('FINFreightAuditVendor_IndexCtrl');
    ...
    $scope.FINFreightAuditVendor_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            ...
            method: _FINFreightAuditVendor.URL.Read,
    ...
    $scope.cboFINFreight_Options = {
        ...
        change: function (e) {
            ...
            if (cboval == _FINFreightAuditVendor.Data._defStatusID)
                $scope.IsDefaultStatus = true;
            else
                $scope.IsDefaultStatus = false;

            if (cboval == _FINFreightAuditVendor.Data._approvedStatusID)
                $scope.IsApprovedStatus = true;
            else
                $scope.IsApprovedStatus = false;

            if (cboval == _FINFreightAuditVendor.Data._waitingStatusID)
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.FIN,
        method: _FINFreightAuditVendor.URL.StatusList,
        success: function (res) {
            if (Common.HasValue(res)) {
                $scope.cboFINFreight_Options.dataSource.data(res);
                _FINFreightAuditVendor.Data._dataStatus = res;
                _FINFreightAuditVendor.Data._defStatusID = res[0].ID;
                _FINFreightAuditVendor.Data._approvedStatusID = res[res.length - 2].ID;
                _FINFreightAuditVendor.Data._waitingStatusID = res[res.length - 3].ID;
    ...
    $scope.Search_Click = function ($event) {
        ...
        Common.Cookie.Set(_FINFreightAuditVendor.Data.CookieSearch, JSON.stringify($scope.ItemFilter));
    ...
    $scope.FreightAudit_Excel = function ($event, grid, win) {
        ...
        $rootScope.UploadExcel({
            ...
            Download: function () {
                Common.Services.Call($http, {
                    url: Common.Services.url.FIN,
                    method: _FINFreightAuditVendor.URL.FreightAuditExport,
            ...
            Upload: function (e, callback) {
                Common.Services.Call($http, {
                    url: Common.Services.url.FIN,
                    method: _FINFreightAuditVendor.URL.FreightAuditCheck,
            ...
            Complete: function (e, data) {
                ...
                if (check)
                    Common.Services.Call($http, {
                        url: Common.Services.url.FIN,
                        method: _FINFreightAuditVendor.URL.FreightAuditImport,
                        data: { data: data },
                        success: function (res) {
    ...
    $scope.Init_LoadCookie = function () {
        ...
        var strCookie = Common.Cookie.Get(_FINFreightAuditVendor.Data.CookieSearch);
        var statusID = -1;
        if (Common.HasValue(strCookie) && strCookie != '') {
            try {
                ...
                if (statusID != _FINFreightAuditVendor.Data._defStatusID)
                    $scope.IsDefaultStatus = false;
                else
                    $scope.IsDefaultStatus = true;
            } catch (e) { }
        ...
        if (!Common.HasValue($scope.ItemFilter.DateFrom) || !Common.HasValue($scope.ItemFilter.DateTo)) {
            ...
            $scope.ItemFilter.DateFrom = new Date().addDays(-2);
            $scope.ItemFilter.DateTo = new Date().addDays(0);
            statusID = _FINFreightAuditVendor.Data._defStatusID;
        }
        ...
        $timeout(function () {
            ...
            if (statusID == _FINFreightAuditVendor.Data._approvedStatusID)
                $scope.IsApprovedStatus = true;
            else
                $scope.IsApprovedStatus = false;

            if (statusID == _FINFreightAuditVendor.Data._waitingStatusID)
                $scope.IsWaitingStatus = true;
            else
                $scope.IsWaitingStatus = false;
    ...
    $scope.Detail_Click = function ($event, grid, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FIN,
            method: _FINFreightAuditVendor.URL.Detail,
            data: { id: item.id },
            success: function (res) {
                if (Common.HasValue(res)) {
    ...
    $scope.Reject_Click = function ($event, win, grid) {
        ...
        $event.preventDefault();
        $scope.Method = _FINFreightAuditVendor.URL.Reject;
        $scope.Item.Note = "";
    ...
    $scope.Accept_Click = function ($event, win, grid) {
        $event.preventDefault();
        $scope.Method = _FINFreightAuditVendor.URL.Accept;
        $scope.Item.Note = "";
    ...
    $scope.Waiting_Click = function ($event, win, grid) {
        $event.preventDefault();
        $scope.Method = _FINFreightAuditVendor.URL.Waiting;
        $scope.Item.Note = "";
        $scope.NoteName = "đồng ý";
    ...
    $scope.Approved_Click = function ($event, win, grid) {
        $event.preventDefault();
        $scope.Method = _FINFreightAuditVendor.URL.Approved;
        $scope.Item.Note = "";
