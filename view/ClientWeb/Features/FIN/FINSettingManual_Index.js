...
var _FINSettingManual_Index = {
    URL: {
        Read: 'FINSettingManual_List',
        Delete: 'FINSettingManual_Delete'
    }
}

angular.module('myapp').controller('FINSettingManual_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('FINSettingManual_IndexCtrl');
    $rootScope.IsLoading = false;

    $scope.setting_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.FIN,
            method: _FINSettingManual_Index.URL.Read,
            model: {
                id: 'SettingID',
                fields: {
    ...
    $scope.CUSSetting_EditClick = function ($event, data) {
        ...
        $state.go("main.FINManualFix.SettingDetail", { id: data.SettingID })
    }
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.FIN,
                    method: _FINSettingManual_Index.URL.Delete,
                    data: { item: data },
    ...
    $scope.AddNew_Click = function ($event) {
        $event.preventDefault();
        $state.go("main.FINManualFix.SettingDetail", { id: 0 })
    }
