...
//#region Data
var _FINFreightAuditCUSCON = {
    URL: {
        Read: "FINFreightAuditCus_OrderCON_List",
        //Detail: "FINFreightAuditCus_Order_DetailList",
        Detail: "FINFreightAuditCus_OrderCONDetail_List",
        Reject: "FINFreightAuditCus_OrderCON_Reject",
        Accept: "FINFreightAuditCus_OrderCON_Accept",
        Approved: "FINFreightAuditCusCON_Order_Approved",
        ...
        CONDetail_Save: 'FINFreightAuditCus_OrderCONDetail_Save',
    },
    Data: {
        CookieSearch: "FINFreightAuditCusCON_Order_CookieSearch",
        _dataStatus: [],
        _defStatusID: -1,
        _approvedStatusID: -1,
     },
...
angular.module('myapp').controller('FINFreightAuditCustomerCON_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('FINFreightAuditCustomerCON_IndexCtrl');
    ...
    $scope.FINFreightAuditCus_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            ...
            method: _FINFreightAuditCUSCON.URL.Read,
    ...
    $scope.cboFINFreight_Options = {
        ...
        change: function () {
            ...
            $timeout(function () {
                ...
                if (cboval == _FINFreightAuditCUSCON.Data._defStatusID)
                    $scope.IsDefaultStatus = true;
                else
                    $scope.IsDefaultStatus = false;

                if (cboval == _FINFreightAuditCUSCON.Data._approvedStatusID)
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.FIN,
        method: _FINFreightAuditCUSCON.URL.StatusList,
        success: function (res) {
            if (Common.HasValue(res)) {
                $scope.cboFINFreight_Options.dataSource.data(res);
                _FINFreightAuditCUSCON.Data._dataStatus = res;
                _FINFreightAuditCUSCON.Data._defStatusID = res[0].ID;
                _FINFreightAuditCUSCON.Data._approvedStatusID = res[res.length - 1].ID;
    ...
    $scope.Search_Click = function ($event) {
        ...
        Common.Cookie.Set(_FINFreightAuditCUSCON.Data.CookieSearch, JSON.stringify($scope.ItemFilter));
    ...
    $scope.Init_LoadCookie = function () {
        ...
        var strCookie = Common.Cookie.Get(_FINFreightAuditCUSCON.Data.CookieSearch);
        var statusID = -1;
        if (Common.HasValue(strCookie) && strCookie != '') {
            try {
                ...
                if (statusID != _FINFreightAuditCUSCON.Data._defStatusID)
        ...
        if (!Common.HasValue($scope.ItemFilter.DateFrom) || !Common.HasValue($scope.ItemFilter.StatusID) || !Common.HasValue($scope.ItemFilter.DateTo)) {
            ...
            $scope.ItemFilter.DateFrom = new Date().addDays(-2);
            $scope.ItemFilter.DateTo = new Date().addDays(0);
            statusID = _FINFreightAuditCUSCON.Data._defStatusID;
        }
    ...
    $scope.Detail_Click = function ($event, grid, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.FIN,
            method: _FINFreightAuditCUSCON.URL.Detail,
            data: { id: item.id },
            success: function (res) {
                if (Common.HasValue(res)) {
                    $scope.ItemContainer = res;
                    $scope.ItemContainer.ID = item.id;
                    ...
                    $scope.LoadDataContractTerm($scope.cboContractTermOptions, res.ContractTermID);
    ...
    $scope.Reject_Click = function ($event, win, grid) {
        $event.preventDefault();
        $scope.Method = _FINFreightAuditCUSCON.URL.Reject;
        $scope.Item.Note = "";
    ...
    $scope.Accept_Click = function ($event, win, grid) {
        $event.preventDefault();
        $scope.Method = _FINFreightAuditCUSCON.URL.Accept;
        $scope.Item.Note = "";
    ...
    $scope.Approved_Click = function ($event, win, grid) {
        $event.preventDefault();
        $scope.Method = _FINFreightAuditCUSCON.URL.Approved;
        $scope.Item.Note = "";
    ...
    $scope.LoadDataContractTerm = function (cbo, contractID) {
        Common.Services.Call($http, {
            url: Common.Services.url.FIN,
            method: _FINFreightAuditCUSCON.URL.ContractTerm_List,
    ...
    $scope.LoadDataRouting = function (cbo, cusID, termID, contractID) {
        Common.Services.Call($http, {
            url: Common.Services.url.FIN,
            method: _FINFreightAuditCUSCON.URL.Routing_List,
    ...
    $scope.Change_Container_Submit_Click = function ($event, win) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.FIN,
                    method: _FINFreightAuditCUSCON.URL.CONDetail_Save,
                    data: { item: $scope.ItemContainer },
                    success: function (res) {
                        Common.Services.Error(res, function (res) {
                            $rootScope.Message({ Msg: "Đã cập nhật!" });
                            $rootScope.IsLoading = false;
                            $scope.FINFreightAuditCus_grid.dataSource.read();
                            win.close();
                        })
                    }
                });
