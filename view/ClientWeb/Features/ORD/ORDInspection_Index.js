...
var _ORDInspection_Index = {
    URL: {
        ...
        DocumentService_Save: "ORDOrder_DocumentService_Save",
        DocumentDetail_List: "ORDOrder_DocumentDetail_List",
        DocumentDetail_SaveList: "ORDOrder_DocumentDetail_SaveList",
        DocumentDetail_DeleteList: "ORDOrder_DocumentDetail_DeleteList",
...
angular.module('myapp').controller('ORDInspection_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('ORDInspection_IndexCtrl');
    ...
    $scope.HasDocumentDetail = false;
    $scope.documentServiceID = 0;
    $scope.GroupInspecID = {ContractCustomerID: null,HasVendor: null};

    $scope.ORDInspection_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.ORD,
            method: _ORDInspection_Index.URL.List,
            readparam: function () {
                return {
        ...
        columns: [
            {
                title: ' ', width: '85px',
                headerTemplate: '<input class="chkSelectAll" type="checkbox" ng-click="gridChooseAll_Check($event,ORDInspection_grid,ORDInspection_gridChooseChange)" />',
                template: '<input class="chkChoose" type="checkbox" #= IsChoose ? "checked=checked" : "" # ng-click="gridChoose_Check($event,ORDInspection_grid,ORDInspection_gridChooseChange)" />' + '<a href="/" ng-click="Edit_Click($event,dataItem,ORDInspection_win)" class="k-button" data-title="Tìm"><i class="fa fa-pencil"></i></a>',
           ...
           {
               field: 'DocumentStatusName', width: 120, title: 'Trạng thái dịch vụ',
               filterable: { cell: { operator: 'contains', showOperators: false } }
           },
            {
            ...
            },
            {
             ...
             {
                 field: 'VendorCode', width: 120, title: 'Mã đối tác',
                 filterable: { cell: { operator: 'contains', showOperators: false } }
             },
            {
                field: 'VendorName', width: 120, title: 'Tên đối tác',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
    ...
    $scope.Search_Click = function ($event) {
        ...
        $scope.ORDInspection_gridOptions.dataSource.read();
    };

    $scope.Inspection_Add_Click = function ($event, win) {
        $event.preventDefault();
        $scope.documentID = 0;
        $scope.LoadData_Inspection(0, win);
    };
    ...
    $scope.Edit_Click = function ($event, data, win) {
        ...
        $scope.ContractVendorID = data.ContractVendorID;
        $scope.load_Service_List();
        $scope.LoadData_Inspection(data.ID, win);
    };

    $scope.LoadData_Inspection = function (ID, win) {
        $rootScope.IsLoading = true;
        Common.Services.Call($http, {
            url: Common.Services.url.ORD,
            method: _ORDInspection_Index.URL.Document_Get,
            data: { ID: ID },
            success: function (res) {
                if (Common.HasValue(res)) {
                    $scope.Item = res;
                    if (res.ContractCustomerID == null) {
                        $scope.ORDService_grid.showColumn($scope.ORDService_grid.columns[3]);
                    } else $scope.ORDService_grid.hideColumn($scope.ORDService_grid.columns[3]);
                    if (res.VendorID != null && res.ContractVendorID == null) {
                        $scope.ORDService_grid.showColumn($scope.ORDService_grid.columns[4]);
                    } else $scope.ORDService_grid.hideColumn($scope.ORDService_grid.columns[4]);
                    win.center().open();
                    $rootScope.IsLoading = false;
    ...
    $scope.Inspection_Delete_Click = function ($event, grid) {
        $event.preventDefault();
        $rootScope.Message({
            Msg: "Xác nhận xóa?",
            ...
            Ok: function () {
                ...
                if (Common.HasValue(data)) {
                    Common.Services.Call($http, {
                        url: Common.Services.url.ORD,
                        method: _ORDInspection_Index.URL.DeleteList,
                        data: {lstID: data },
                        success: function (res) {
                            $scope.ORDInspection_grid.dataSource.read();
                            $rootScope.Message({
                                Msg: 'Đã xóa.',
    ...
    $scope.Load_cboTypeOfDocument = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.ORD,
            method: _ORDInspection_Index.URL.CustomerList,
            success: function (res) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.ORD,
            method: _ORDInspection_Index.URL.Document_Read,
            success: function (res) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.ORD,
            method: _ORDInspection_Index.URL.VendorList,
            success: function (res) {
    ...
    $scope.Inspection_Save_Click = function ($event, win, grid, vform) {
        ...
        if (vform()) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.ORD,
                method: _ORDInspection_Index.URL.Document_Save,
