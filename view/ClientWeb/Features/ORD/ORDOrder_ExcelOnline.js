...
var _ORDOrder_ExcelOnline = {
    URL: {
        ...
        Export: 'ORDOrder_ExcelOnline_Export',

        //GOP
        GOPRead: "GroupOfProduct_Read",
        Product_Create: "ORDOrder_Excel_Product_Create",

        ExcelOnline_Init: 'ORDOrder_ExcelOnline_Init',
        ...
        ExcelOnline_LocationToSave: 'ORDOrder_ExcelOnline_LocationToSave',
    },
    Data: {
        Province: [],
        District: [],
        ...
        CookieSearch: "_ORDOrder_ExcelOnline_Template",
        CheckResult: [],
    },
    iFCL: 31,
    iFTL: 33, 
...
angular.module('myapp').controller('ORDOrder_ExcelOnlineCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('ORDOrder_ExcelOnlineCtrl');
    ...
    $scope.excel_template_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            ...
            method: _ORDOrder_ExcelOnline.URL.Template_List,
            pageSize: 0,
            model: {
                id: 'ID',
                fields: {
            ...
            $(grid.items()).on('dblclick', function (e) {
                ...
                if (Common.HasValue(item)) {
                    $timeout(function () {
                        ...
                        Common.Cookie.Set(_ORDOrder_ExcelOnline.Data.CookieSearch, JSON.stringify($scope.Template));
    ...
    $scope.Init_LoadCookie = function () {
        ...
        var strCookie = Common.Cookie.Get(_ORDOrder_ExcelOnline.Data.CookieSearch);
    ...
    $scope.ExcelOnline_Click = function () {
        ...
        var lstMessageError = [];
        for (var i = 0; i < 15; i++) {
            var resource = $rootScope.RS[_ORDOrder_ExcelOnline.ExcelKey.Resource + '_' + i];
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.ORD,
            method: _ORDOrder_ExcelOnline.URL.Template_Get,
            data: {
            ...
            success: function (res) {
                ...
                if (Common.HasValue(res)) {
                    $scope.File = { FileName: 'ORDOrder_ExcelOnline', FileExt: 'xlsx', FilePath: '', ReferID: $rootScope.FunctionItem.ID, TypeOfFileCode: Common.CATTypeOfFileCode.ORD };
                    
                    $scope.excelOrder.Init({
                        functionkey: _ORDOrder_ExcelOnline.ExcelKey.Order + $scope.Template.ID + "_" + $scope.Template.CustomerID + "_" + new Date().getTime(),
                        params: {
                        ...
                        url: Common.Services.url.ORD,
                        methodInit: _ORDOrder_ExcelOnline.URL.ExcelOnline_Init,
                        methodChange: _ORDOrder_ExcelOnline.URL.ExcelOnline_Change,
                        methodImport: _ORDOrder_ExcelOnline.URL.ExcelOnline_Import,
                        methodApprove: _ORDOrder_ExcelOnline.URL.ExcelOnline_Approve,
    ...
    $timeout(function () {
        if (Common.HasValue($('#spreadsheetOrder'))) {
            $scope.excelOrder.data.spreadsheet = $('#spreadsheetOrder').kendoSpreadsheet({
                excelImport: function (e) {
                    if (e.file.size > 3000000) {
                        ...
                        $scope.excelOrder.data.isImport = true;
                        e.promise.progress(function (e) { })
                        .done(function (res) {
                            $rootScope.IsLoading = true;

                            var data = { id: $scope.excelOrder.data.item.ID, worksheets: $scope.excelOrder.data.spreadsheet.sheets(), lstMessageError: $scope.excelOrder.data.options.lstMessageError };
                            angular.forEach($scope.excelOrder.data.options.params, function (v, p) {
                                data[p] = v;
                            });
                            Common.Services.Call($http, {
                                url: $scope.excelOrder.data.options.url,
                                method: $scope.excelOrder.data.options.methodImport,
                                data: data,
                                success: function (res) {
                                    $rootScope.IsLoading = false;

                                    $scope.excelOrder.data.spreadsheet.fromJSON({ sheets: res.SYSExcel.Worksheets });
                                    $scope.excelOrder.data.isImport = false;
                                    $scope.excelOrder.data.isChange = true;
                ...
                change: function (arg) {
                    if (!$scope.excelOrder.data.isImport) {
                        var lstRowCheck = [];
                        var sheet = $scope.excelOrder.data.spreadsheet.activeSheet();
                        var rowChange = -1;
                        var colChange = -1;
                        var valChange = '';
                        ...
                        arg.range.forEachCell(function (row, col, value) {
                            if (Common.HasValue(value) && Common.HasValue(value.value)) {
                                ...
                                colFail++;
                            if (col < $scope.excelOrder.data.options.colCheckChange) {
                                rowChange = row;
                                colChange = col;
                        ...
                        if (colFail < 2) {
                            if (lstRowCheck.length > 0) {
                                angular.forEach(lstRowCheck, function (rowindex, p) {
                                    ...
                                    $scope.excelOrder.data.rowRunning++;
                                    $scope.excelOrder.data.isChange = true;
                                    $scope.excelOrder.LoadActive();
                                    var data = { id: $scope.excelOrder.data.item.ID, row: rowindex, cells: cells, lstMessageError: $scope.excelOrder.data.options.lstMessageError };
                                    angular.forEach($scope.excelOrder.data.options.params, function (v, p) {
                                        data[p] = v;
                                    });
                                    Common.Services.Call($http, {
                                        url: $scope.excelOrder.data.options.url,
                                        method: $scope.excelOrder.data.options.methodChange,
                                        data: data,
                                        success: function (res) {
                                            $scope.excelOrder.data.rowRunning--;
                                            ...
                                            if (Common.HasValue($scope.excelOrder.data.options.Changed))
                                                $scope.excelOrder.data.options.Changed();
                                            $scope.excelOrder.LoadActive();
                                        ...
                                        error: function (res) {
                                            $scope.excelOrder.data.rowRunning--;
                                            $scope.excelOrder.data.isChange = false;
                                            $rootScope.IsLoading = false;
                                        }
                                    });
                                });
    ...
    $scope.excelOrder = {
        ...
        LoadActive: function () {
            if ($scope.excelOrder.data.rowRunning > 0) {
                $scope.excelOrder.data.activeAccept = true;
                $scope.excelOrder.data.captionAccept = '(' + $scope.excelOrder.data.rowRunning + ') kiểm tra';
            }
            else {
                $scope.excelOrder.data.activeAccept = false;
                $scope.excelOrder.data.captionAccept = 'Đồng ý';
            }
        },
        Init: function (options) {
            ...
            if (!Common.HasValue(options.methodInit) || options.methodInit == '')
                ...
                $scope.excelOrder.data.options = options;

                //$scope.winexcelshare.center();
                //$scope.winexcelshare.maximize();
                //$scope.winexcelshare.open();

                var data = { functionid: $rootScope.FunctionItem.ID, functionkey: $scope.excelOrder.data.options.functionkey, isreload: false };
                angular.forEach($scope.excelOrder.data.options.params, function (v, p) {
                    data[p] = v;
                });
                $rootScope.IsLoading = true;
                Common.Services.Call($http, {
                    ...
                    success: function (res) {
                        $scope.excelOrder.data.isImport = true;
                        $scope.excelOrder.data.item = res;
                        $scope.excelOrder.data.spreadsheet.fromJSON({ sheets: res.Worksheets });
                        $scope.excelOrder.data.isImport = false;
    ...
    $scope.excelOrder_Approve_Click = function ($event) {
        ...
        if ($scope.excelOrder.data.rowRunning == 0) {
            if ($scope.excelOrder.data.isChange == true) {
                $rootScope.Message({
                    ...
                    Ok: function (pars) {
                        ...
                        var data = { id: $scope.excelOrder.data.item.ID };
                        angular.forEach($scope.excelOrder.data.options.params, function (v, p) {
                            data[p] = v;
                        });
                        Common.Services.Call($http, {
                            url: $scope.excelOrder.data.options.url,
                            method: $scope.excelOrder.data.options.methodApprove,
                            data: data,
                            success: function (res) {
                                $scope.excelOrder.data.isChange = false;
                                $rootScope.Loading.Change('Lấy dữ liệu excel mới', 10);
                                if (res.ID > 0) {
                                    var data = { functionid: $rootScope.FunctionItem.ID, functionkey: $scope.excelOrder.data.options.functionkey, isreload: true };
                                    angular.forEach($scope.excelOrder.data.options.params, function (v, p) {
                                        data[p] = v;
                                    });
                                    ...
                                    Common.Services.Call($http, {
                                        url: $scope.excelOrder.data.options.url,
                                        method: $scope.excelOrder.data.options.methodInit,
                                        data: data,
                                        success: function (res) {
                                            ...
                                            if (Common.HasValue($scope.excelOrder.data.options.Approved))
                                                $scope.excelOrder.data.options.Approved();
    ...
    $scope.Location_AddClick = function ($event) {
        $event.preventDefault();
        $scope.Location_Click($event, $scope.excel_location_win, _ORDOrder_ExcelOnline.Data.CheckResult);
    }
    ...
    $scope.Location_Click = function ($event, win, data) {
        ...
        if (_ORDOrder_ExcelOnline.Data.Province.length > 0) {
            $.each(dataRes, function (i, o) {
                o.DataProvince = _ORDOrder_ExcelOnline.Data.Province;
                if (o.ProvinceID == null || o.ProvinceID <= 0)
                    o.ProvinceID = _ORDOrder_ExcelOnline.Data.Province[0].ID;
                o.DataDistrict = $.grep(_ORDOrder_ExcelOnline.Data.District, function (v) { return o.ProvinceID == v.ProvinceID });
    ...
    $scope.onLocation_ProvinceChange = function (event, item) {
        ...
        if (Common.HasValue(obj)) {
            var data = $.grep(_ORDOrder_ExcelOnline.Data.District, function (o) { return o.ProvinceID == obj.ID });
    ...
    Common.ALL.Get($http, {
        ...
        success: function (res) {
            _ORDOrder_ExcelOnline.Data.Province = res;
        }
    });
    ...
    Common.ALL.Get($http, {
        ...
        success: function (res) {
            _ORDOrder_ExcelOnline.Data.District = res;
        }
    });
        ...
        if (isValid && !isError) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.ORD,
                        method: _ORDOrder_ExcelOnline.URL.Location_Create,
                        data: {
                            dataLocation: data
                        },
                        success: function (res) {
    ...
    $scope.Product_AddClick = function ($event) {
        $event.preventDefault();
        $scope.Product_Click($event, $scope.excel_product_win, _ORDOrder_ExcelOnline.Data.CheckResult);
    }
    ...
    $scope.Product_Click = function ($event, win, data) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.ORD,
            method: _ORDOrder_ExcelOnline.URL.GOPRead,
            data: {
                id: cusID,
            },
            success: function (res) {
        ...
        if (isValid) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.ORD,
                        method: _ORDOrder_ExcelOnline.URL.Product_Create,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if (data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.ORD,
                        method: _ORDOrder_ExcelOnline.URL.ExcelOnline_LocationToSave,
                        data: {
                            id: $scope.excelOrder.data.item.ID,
                            templateID: $scope.Template.ID,
                            lst: data,
                        },
                        success: function (res) {
