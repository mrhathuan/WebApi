...
//#region URL
var _KPITimeDate_Index = {
    URL: {
        Time_List: 'KPITimeDate_List',
        Time_Save: 'KPITimeDate_Save',
        Time_Excel: 'KPITimeDate_Excel',
        Time_Generate: 'KPITimeDate_Generate',
        Time_Reason_List: 'KPITimeDateReason_List',
        Customer_List: 'Customer_List',

        KPI_GetTypeOfKPI: 'KPITimeDate_GetTypeOfKPI',
...
angular.module('myapp').controller('KPITimeDate_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('KPITimeDate_IndexCtrl');
    ...
    $scope.TimeItem = null;
    $scope.ViewItem = {
        CusID: -1, KPIID: -1, DateFrom: new Date().addDays(-3), DateTo: new Date().addDays(3)
    };
    ...
    $scope.DashBoardDN_Item = {
        ...
        CustomerID: -1, KPIID: -1, CustomerName: 'Tất cả KH', KPIName: '', IsRadialGauge: false
    };

    $scope.DashBoardReason_Item = {
        KPIID: -1, CustomerID: -1, CustomerName: 'Tất cả KH', KPIName: '',
        StrFrom: Common.Date.ToString(new Date().addDays(-3), Common.Date.Format.DMY),
        StrTo: Common.Date.ToString(new Date().addDays(3), Common.Date.Format.DMY),
            ...
            $timeout(function () {
                ...
                switch (obj.ID) {
                    ...
                    case 1:
                        ...
                        data = _KPITimeDate_Index.Data.DashBoard_Reason_1;
                        break;
                    case 2:
                        options = {
                            series: [{
                        ...
                        data = _KPITimeDate_Index.Data.DashBoard_Reason_100;
                        break;
                    case 3:
                        options = {
                            series: [{
    ...
    $scope.DashBoard_DN_Setting_OK = function ($event) {
        ...
        Common.Services.Call($http, {
            ...
            method: _KPITimeDate_Index.URL.DashBoard_DN,
            data: {
    ...
    $scope.DashBoard_Reason_Setting_OK = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.DashBoard_Reason,
            data: {
            ...
            success: function (res) {
                ...
                _KPITimeDate_Index.Data.DashBoard_Reason_1 = data_1;
                _KPITimeDate_Index.Data.DashBoard_Reason_100 = data_100;
                $timeout(function () {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.KPI,
        method: _KPITimeDate_Index.URL.Customer_List,
        success: function (res) {
            var data = [{ ID: -1, CustomerName: "Tất cả KH" }];
            Common.Data.Each(res.Data, function (o) {
                data.push(o);
                if ($scope.ViewItem.CusID < 1)
                    $scope.ViewItem.CusID = o.ID;
            })
            $timeout(function () {
                $scope.cboTimeDate_CusOptions.dataSource.data(res.Data);
    ...
    $scope.kpi_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.Time_List,
            readparam: function () {
                return {
                    cusID: $scope.ViewItem.CusID, kpiID: $scope.ViewItem.KPIID,
    ...
    $scope.Generate_Click = function ($event) {
        ...
        $rootScope.Message({
            ...
            Ok: function (pars) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.KPI,
                    method: _KPITimeDate_Index.URL.Time_Generate,
                    data: pars,
                    success: function (res) {
                        $rootScope.IsLoading = false;
    ...
    $scope.LoadData_Reason = function () {
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.Time_Reason_List,
    ...
    $scope.Time_Reason_Submit_Click = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.Time_Save,
    ...
    $scope.KPI_Excel_Click = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.Time_Excel,
            data: {
                cusID: $scope.ViewItem.CusID, kpiID: $scope.ViewItem.KPIID,
                from: $scope.ViewItem.DateFrom, to: $scope.ViewItem.DateTo
            },
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.KPI,
        method: _KPITimeDate_Index.URL.KPI_GetTypeOfKPI,
        success: function (res) {
            _KPITimeDate_Index.Data.KPI = res;
            Common.Data.Each(res, function (o) {
                if (_KPITimeDate_Index.Item.Reason.KPIID < 1)
                    _KPITimeDate_Index.Item.Reason.KPIID = o.ID;
                ...
                if ($scope.DashBoardReason_Item.KPIID < 1) {
                    ...
                    $scope.DashBoardReason_Item.KPIName = o.TypeName;
                }
                if ($scope.DashBoardDN_Item.KPIID < 1) {
                    $scope.DashBoardDN_Item.KPIID = o.ID;
                    $scope.DashBoardDN_Item.KPIName = o.TypeName;
                }
            })

            $timeout(function () {
                $scope.cboReasonKPI_Options.dataSource.data(_KPITimeDate_Index.Data.KPI);
                $scope.cboTimeDate_KPIOptions.dataSource.data(_KPITimeDate_Index.Data.KPI);
                $scope.cboDashBoard_KPIOptions.dataSource.data(_KPITimeDate_Index.Data.KPI);
    ...
    $scope.reason_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.Reason_List,
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number' }
    ...
    $scope.Reason_Add_Click = function ($event, grid, win) {
        ...
        $scope.ReasonItem = $.extend({}, true, _KPITimeDate_Index.Item.Reason);
        win.center().open();
    }
    ...
    $scope.Reason_Delete_Click = function ($event, item, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPITimeDate_Index.URL.Reason_Delete,
            data: { id: item.ID },
            success: function (res) {
    ...
    $scope.Reason_Submit_Click = function ($event, grid, win, vform) {
        ...
        if (vform()) {
            Common.Services.Call($http, {
                url: Common.Services.url.KPI,
                method: _KPITimeDate_Index.URL.Reason_Save,
    ...
    $scope.Reason_Excel_Click = function ($event, win) {
        ...
        $rootScope.UploadExcel({
            ...
            Download: function () {
                Common.Services.Call($http, {
                    url: Common.Services.url.KPI,
                    method: _KPITimeDate_Index.URL.Reason_Export,
                    success: function (res) {
                        $rootScope.DownloadFile(res);
                    }
                })
            ...
            Upload: function (e, callback) {
                Common.Services.Call($http, {
                    url: Common.Services.url.KPI,
                    method: _KPITimeDate_Index.URL.Reason_Check,
            ...
            Complete: function (e, data) {
                Common.Services.Call($http, {
                    url: Common.Services.url.KPI,
                    method: _KPITimeDate_Index.URL.Reason_Import,
                    data: { data: data },
                    success: function (res) {
                        $rootScope.Message({ Msg: "Đã cập nhật!" });
