...
//#region URL
var _KPIVENTenderFTL_Index = {
    URL: {
        VENTender_List: 'KPIVENTenderFTL_List',
        VENTender_Save: 'KPIVENTenderFTL_Save',
        VENTender_Excel: 'KPIVENTenderFTL_Excel',
        VENTender_Generate: 'KPIVENTenderFTL_Generate',
...
angular.module('myapp').controller('KPIVENTenderFTL_IndexCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('KPIVENTenderFTL_IndexCtrl');
    ...
    $scope.kpi_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.KPI,
            method: _KPIVENTenderFTL_Index.URL.VENTender_List,
            readparam: function () {
                return {
            ...
            model: {
                ...
                fields: {
                    ...
                    TotalAccept: { type: 'number', editable: false },
                    TotalKPI: { type: 'number', editable: false },
                    TotalReject: { type: 'number', editable: false },
                    TotalSchedule: { type: 'number', editable: false },
        ...
        columns: [
            ...
            {
                field: 'TotalAccept', title: 'Số lần chấp nhận', width: '100px',
                filterable: { cell: { operator: 'equal', showOperators: false } }
            },
            {
                field: 'TotalKPI', title: 'Số lần tính KPI', width: '100px',
                filterable: { cell: { operator: 'equal', showOperators: false } }
            },
            {
                field: 'TotalReject', title: 'Số lần từ chối', width: '100px',
                filterable: { cell: { operator: 'equal', showOperators: false } }
            },
            {
                field: 'TotalSchedule', title: 'Tổng chuyến', width: '100px',
                filterable: { cell: { operator: 'equal', showOperators: false } }
            },
            {
    ...
    $scope.Generate_Click = function ($event) {
        ...
        $rootScope.Message({
            ...
            Ok: function (pars) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.KPI,
                    method: _KPIVENTenderFTL_Index.URL.VENTender_Generate,
                    data: pars,
                    success: function (res) {
                        $rootScope.IsLoading = false;
    ...
    $scope.LoadData_Reason = function () {
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPIVENTenderFTL_Index.URL.Time_Reason_List,
    ...
    $scope.VenTender_Reason_Submit_Click = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPIVENTenderFTL_Index.URL.VENTender_Save,
    ...
    $scope.KPI_Excel_Click = function ($event) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.KPI,
            method: _KPIVENTenderFTL_Index.URL.VENTender_Excel,
            data: {
                venID: $scope.ViewItem.VenID,
                from: $scope.ViewItem.DateFrom, to: $scope.ViewItem.DateTo
            },
