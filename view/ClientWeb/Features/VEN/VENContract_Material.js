...
var _VENContract_Material = {
    URL: {
        Data: 'VENContract_MaterialChange_Data',
        Save: 'VENContract_MaterialChange_Save',
    },
    Data: {
        PriceCurrent: null,
        PriceChange: null,
...
angular.module('myapp').controller('VENContract_MaterialCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('VENContract_MaterialCtrl');
    $rootScope.IsLoading = false;

    _VENContract_Material.Params = $.extend( true,_VENContract_Material.Params, $state.params);
    $scope.TabIndex = 0;
    ...
    Common.Services.Call($http, {
        ...
        method: _VENContract_Material.URL.Data,
        data: {  contractTermID: _VENContract_Material.Params.TermID },
        success: function (res) {
            $scope.ItemMaterial = res.TermInfo;
            _VENContract_Material.Data.ContractID = res.TermInfo.ContractID
            //_VENContract_Material.Data.customerID = res.CustomerID;
    ...
    $scope.CreateFTLLevel = function (isframe) {
        ...
        Common.Data.Each($scope.ItemPriceCurrent.FTLLevel.ListDetail, function (o) {
            if (!Common.HasValue(dataCheckCurrent["R" + o.RoutingID + "_L" + o.LevelID]))
                dataCheckCurrent["R" + o.RoutingID + "_L" + o.LevelID] = o;
        })
        var dataGridCurrent = [];
        if (isframe) {
            Common.Data.Each($scope.ItemPriceCurrent.ListRouting, function (route) {
        ...
        if (!Common.HasValue($scope.ItemPriceChange.ItemPrice.Code) || $scope.ItemPriceChange.ItemPrice.Code == '') {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.VEN,
                        method: _VENContract_Material.URL.Save,
                        data: { item: item, contractMaterialID: $scope.ItemMaterial.ID },
                        success: function (res) {
                            $rootScope.IsLoading = false;
                            $state.go("main.VENVendor.Contract", { ID: $scope.ItemMaterial.ContractID, CustomerID: 0 })
                        },
                        error: function (res) {
                            $rootScope.IsLoading = false;
                        }
    ...
    //    if (_VENContract_Material.Params.StatusID > 0) {
    //        $scope.IsShowBackClick = true;
    //    }
    //    else {
    //        $scope.IsShowBackClick = false;
    ...
    $scope.Back_Click = function ($event) {
        $event.preventDefault();
        //if (_VENContract_Material.Params.StatusID == 1) {
        //    $state.go("main.VENContract.Detail", { ID: _VENContract_Material.Data.ContractID });
        //}
        //else if (_VENContract_Material.Params.StatusID == 2) {
        $state.go("main.VENVendor.Contract", { ID: $scope.ItemMaterial.ContractID, CustomerID: 0 })
    };
