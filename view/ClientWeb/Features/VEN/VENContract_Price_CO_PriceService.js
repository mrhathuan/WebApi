...
var _VENContract_Price_CO_PriceService = {
    URL: {
        Read: 'VENPrice_CO_Service_List',
        Get: 'VENPrice_CO_Service_Get',
        Save: 'VENPrice_CO_Service_Save',
        Delete: 'VENPrice_CO_Service_Delete',

        ListCurrency: 'CATCurrency_AllList',
        ListService: 'VENPrice_CO_CATService_List',
    },
    Data: {
    },
    Params: {
...
angular.module('myapp').controller('VENContract_Price_CO_PriceServiceCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('VENContract_Price_CO_PriceServiceCtrl');
    $rootScope.IsLoading = false;
    $scope.Item = null;
    _VENContract_Price_CO_PriceService.Params = $.extend(true, _VENContract_Price_CO_PriceService.Params, $state.params);

    $scope.Service_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.VEN,
            method: _VENContract_Price_CO_PriceService.URL.Read,
            readparam: function () {
                return {
                    priceID: _VENContract_Price_CO_PriceService.Params.PriceID,
                }
            },
            model: {
                id: 'ID',
    ...
    $scope.ServiceLoadItem = function (id, win, vform) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.VEN,
            method: _VENContract_Price_CO_PriceService.URL.Get,
            data: { id: id },
    ...
    $scope.Destroy_Click = function ($event, item) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.VEN,
                    method: _VENContract_Price_CO_PriceService.URL.Delete,
                    data: { ID: item.ID },
                    success: function (res) {
    ...
    $scope.Service_Save_Click = function ($event, win, vform) {
        ...
        if (vform()) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.VEN,
                method: _VENContract_Price_CO_PriceService.URL.Save,
                data: { item: $scope.Item, priceID: _VENContract_Price_CO_PriceService.Params.PriceID},
                success: function (res) {
                    $scope.Service_gridOptions.dataSource.read();
                    $rootScope.IsLoading = false;
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.VEN,
        method: _VENContract_Price_CO_PriceService.URL.ListService,
    ...
    Common.Services.Call($http, {
        ...
        method: _VENContract_Price_CO_PriceService.URL.ListCurrency,
    ...
    $scope.Back_Click = function ($event) {
        $event.preventDefault();
        $state.go("main.VENContract.PriceCO", _VENContract_Price_CO_PriceService.Params)
    }
}]);