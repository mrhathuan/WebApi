...
var _VENContract_Price_DI_UnLoadTypeOfPartner = {
    URL: {
        Get: 'VENContract_Price_Get',
        Data: 'VENContract_Price_Data',
        UnLoading_List: 'VENPrice_DI_UnLoadPartner_List',
        UnLoading_Partner_NotIn_List: 'VENPrice_DI_UnLoadPartner_PartnerNotIn_List',
        UnLoading_Partner_NotIn_SaveList: 'VENPrice_DI_UnLoadPartner_PartnerNotIn_SaveList',
        UnLoading_Delete: 'VENPrice_DI_UnLoad_Delete',
        UnLoading_SaveList: 'VENPrice_DI_UnLoadPartner_SaveList',
...
angular.module('myapp').controller('VENContract_Price_DI_UnLoadTypeOfPartnerCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$stateParams', function ($rootScope, $scope, $http, $location, $state, $timeout, $stateParams) {
    Common.Log('VENContract_Price_DI_UnLoadTypeOfPartnerCtrl');
    $rootScope.IsLoading = true;

    $scope.Item = null;
    _VENContract_Price_DI_UnLoadTypeOfPartner.Params = $.extend({}, true, $state.params);

    Common.ALL.Get($http, {
        url: Common.ALL.URL.SYSVarPriceOfGOP,
        success: function (res) {
            _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListPriceOfGOP = res;
        }
    })

    Common.Services.Call($http, {
        url: Common.Services.url.VEN,
        method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.Data,
        data: { contractTermID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.TermID },
        success: function (res) {
            _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct = res.ListGroupOfProduct;
            Common.Services.Call($http, {
                url: Common.Services.url.VEN,
                method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.Get,
                data: { priceID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.PriceID },
                success: function (res) {
                    $scope.Item = res;
    ...
    $scope.Back_Click = function ($event) {
        ...
        $state.go("main.VENContract.PriceDI", _VENContract_Price_DI_UnLoadTypeOfPartner.Params)
    }
    ...
    $scope.InitView_UnLoading = function () {
        ...
        for (var i = 0; i < _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct.length; i++) {
            var item = _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct[i];
                            ...
                            $('<input data-bind="value:' + options.field + '" />').appendTo(container)
                                .kendoComboBox({
                                    ...
                                    dataSource: Common.DataSource.Local({
                                        data: _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListPriceOfGOP,
                                        model: {
                                            id: 'ID',
                                            fields: {
                                                ID: { type: 'number' },
    ...
    $scope.LoadData_UnLoading = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.VEN,
            method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.UnLoading_List,
            ...
            success: function (res) {
                ...
                Common.Services.Error(res, function (res) {
                    ...
                    for (var i = 0; i < res.length; i++) {
                        ...
                        for (var j = 0; j < _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct.length; j++) {

                            var gop = _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct[j];
                            var field = "F_" + gop.ID
    ...
    $scope.UnLoading_Delete_Click = function ($event, item, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.VEN,
            method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.UnLoading_Delete,
            data: { ID: item.ID },
            success: function (res) {
    ...
    $scope.UnLoading_Update_Click = function ($event, grid) {
        ...
        for (var i = 0; i < dataSource.length; i++) {
            ...
            if (dataitem.dirty) {
                ...
                for (var j = 0; j < _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct.length; j++) {
                    var product = _VENContract_Price_DI_UnLoadTypeOfPartner.Data.ListGroupProduct[j];
        ...
        if (data.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.VEN,
                method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.UnLoading_SaveList,
                data: { data: data },
                success: function (res) {
                    $scope.LoadData_UnLoading();
    ...
    $scope.UnLoading_DeleteAll_Click = function ($event, grid) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.VEN,
                    method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.UnLoading_DeleteList,
                    data: { data: data },
                    success: function (res) {
    ...
    $scope.unloading_partner_notin_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.VEN,
            method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.UnLoading_Partner_NotIn_List,
            readparam: function () {
                return {
    ...
    $scope.UnLoading_Partner_NotIn_Submit_Click = function ($event, grid, win) {
        ...
        if (data.length > 0) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.VEN,
                method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.UnLoading_Partner_NotIn_SaveList,
    ...
    $scope.Loading_Excel_Click = function ($event, win) {
        ...
        $rootScope.UploadExcel({
            ...
            Download: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.VEN,
                    method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.Excel_Export,
                    data: { contractTermID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.TermID, priceID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.PriceID },
                    success: function (res) {
                        $rootScope.IsLoading = false;
            ...
            Upload: function (e, callback) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.VEN,
                    method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.Excel_Check,
                    data: { file: e, contractTermID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.TermID, priceID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.PriceID },
            ...
            Complete: function (e, data) {
                if (!$scope.Item.TermClosed) {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.VEN,
                        method: _VENContract_Price_DI_UnLoadTypeOfPartner.URL.Excel_Save,
                        data: { lst: data, priceID: _VENContract_Price_DI_UnLoadTypeOfPartner.Params.PriceID },
                        success: function (res) {
                            $rootScope.Message({
                                Msg: 'Đã cập nhật.',
