...
var _OPSAppointment_COViewOnMap = {
    URL: {
...
angular.module('myapp').controller('OPSAppointment_COViewOnMapCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', 'openMapV2', function ($rootScope, $scope, $http, $location, $state, $timeout, openMapV2) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COViewOnMapCtrl');
    ...
    $scope.IsShowOrder = false; //Hiển thị popup đơn hàng 
    $scope.IsShowVehicle = false; //Hiển thị popup phương tiện
    ...
    Common.Services.Call($http, {
        ...
        method: _OPSAppointment_COViewOnMap.URL.Setting,
        success: function (res) {
            if (Common.HasValue(res)) {
                ...
                _OPSAppointment_COViewOnMap.Data.Location.LocationStartID = res.LocationStartID;
                _OPSAppointment_COViewOnMap.Data.Location.LocationStartName = res.LocationStartName;
                _OPSAppointment_COViewOnMap.Data.Location.LocationEndID = res.LocationEndID;
                _OPSAppointment_COViewOnMap.Data.Location.LocationEndName = res.LocationEndName;
                _OPSAppointment_COViewOnMap.Data.Location.LocationEndLat = res.LocationEndLat;
                _OPSAppointment_COViewOnMap.Data.Location.LocationEndLng = res.LocationEndLng;
                _OPSAppointment_COViewOnMap.Data.Location.LocationStartLat = res.LocationStartLat;
                _OPSAppointment_COViewOnMap.Data.Location.LocationStartLng = res.LocationStartLng;
                _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocID = res.LocationRomoocGetID;
                _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocName = res.LocationRomoocGetName;
                _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLat = res.LocationRomoocGetLat;
                _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLng = res.LocationRomoocGetLng;
            }
        }
    });
        ...
        ClickMarker: function (i, o) {
            switch (i.Type) {
                case 'Romooc':
                    ...
                    if ($scope.IsShowVehicle == false) {
                        $scope.IsShowVehicle = true;

                        var oMap = $('#map').offset();
                        var w = typeof ($scope.vehicle_win.options.width) == 'string' && $scope.vehicle_win.options.width.endsWith('px') ? $scope.vehicle_win.options.width.substr(0, $scope.vehicle_win.options.width.length - 2) : $scope.vehicle_win.options.width;
                        $scope.vehicle_win.setOptions({
                            position: {
                                top: oMap.top + 30, left: oMap.left + $('#map').width() - 10 - w
                            }
                        });

                        $scope.vehicle_win.open();
                    }
                    $timeout(function () {
                        $scope.romooc_Grid.resize();
                    }, 200);
                    break;
                case 'Tractor':
                    $scope.LocationItem = null;
                    ...
                    Common.Data.Each($scope.tractor_Grid.items(), function (tr) {
                        var item = $scope.tractor_Grid.dataItem(tr);
                        $(tr).removeClass('k-state-selected');
                        if (Common.HasValue(item) && item.ID == $scope.VehicleItem.ID) {
                            $(tr).addClass('k-state-selected');
                        }
    ...
    $scope.con_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.Order_List,
            pageSize: 20,
            readparam: function () {
                return {
    ...
    $scope.romooc_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.Romooc_List,
            readparam: function () {
                return {
        ...
        selectable: 'row', filterable: { mode: 'row', visible: false }, reorderable: false, autoBind: true,
        change: function (e) {
    ...
    $scope.tractor_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.Tractor_List,
            readparam: function () {
                return {
                    requestDate: $scope.VehicleRequestDate
                }
    ...
    $scope.VehicleWinWidth = 350;

    $scope.OnVehiclePopupResize = function (win) {        
        $scope.VehicleWinWidth = typeof (win.options.width) == 'string' && win.options.width.endsWith('px') ? win.options.width.substr(0, win.options.width.length - 2) : win.options.width;
    }
    ...
    $scope.location_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.Location_List,
            pageSize: 20,
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.$watch('TripActived', function (n, o) {
        if ($scope.TripActived) {
            ...
            $scope.vehicle_win.close();
            $scope.TripDetail = false;
            $scope.IsShowOrder = false;
            $scope.IsShowVehicle = false;
    ...
    $scope.romoocVendor_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.VehicleVendor_List,
            readparam: function () {
                return {
    ...
    $scope.VehicleVendor_New_OK_Click = function ($event, grid, win) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _OPSAppointment_COViewOnMap.URL.New_Vendor_Vehicle_Save,
                    data: {
    ...
    //Ẩn/hiện popup đơn hàng
    $scope.Order_Click = function ($event) {
        $event.preventDefault();

        if ($scope.IsShowOrder == false) {
            $scope.IsShowOrder = true;

            $scope.ShowOrderDate = false;
            var oMap = $('#map').offset();
            $scope.con_win.setOptions({
                position: {
                    top: oMap.top + 10, left: oMap.left + 10
                }
            });
            $scope.con_win.open();
    ...
    //Ẩn/hiện popup phương tiện
    $scope.Vehicle_Click = function ($event) {
        $event.preventDefault();
    ...
    $scope.ViewVehicleInfo = function ($event, item, type) {
        ...
        if (!Common.HasValue(_OPSAppointment_COViewOnMap.Data.VehicleInfo[type + "_" + item.ID])) {
            _OPSAppointment_COViewOnMap.Data.VehicleInfo[type + "_" + item.ID] = {
                IsLoaded: false,
                ListTrip: []
            }
            ...
            $scope.LoadVehicleTrip(new Date(), type == 1 ? item.ID : -1, type == 2 ? item.ID : -1, 3, true, true, function (res) {
                ...
                var obj = _OPSAppointment_COViewOnMap.Data.VehicleInfo[type + "_" + item.ID];
                $scope.VehicleInfo = {
    ...
    $scope.LoadVehicleTrip = function (requestDate, vehicleID, romoocID, total, isapproved, istendered, callback) {
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.TripByVehicle_List,
            data: {
    ...
    $scope.TripViewDate_Click = function ($event, grid) {
        $event.preventDefault();
        if ($event.currentTarget == $event.target)
            $scope.ShowTripDate = !$scope.ShowTripDate;
    }

    $scope.TripViewDate_Cancel_Click = function ($event, grid) {
        $event.preventDefault();
        $scope.ViewTripDate = false;
        $scope.ShowTripDate = false;
        $scope.TripDateRequest = { fDate: null, tDate: null };
        grid.dataSource.read();
    }

    $scope.TripViewDate_OK_Click = function ($event, grid) {
        $event.preventDefault();

        $scope.ViewTripDate = true;
        $scope.ShowTripDate = false;
        grid.dataSource.read();
    }

    $scope.$watch('VehicleTripRequest.Date', function (n, o) {
        if (o != n) { //Disable When Init
            $timeout(function () {
                $scope.LoadVehicleTrip($scope.VehicleTripRequest.Date, $scope.VehicleTripRequest.VehicleID, $scope.VehicleTripRequest.RomoocID, -1, $scope.ViewVehicleTripApproved, $scope.ViewVehicleTripTendered, function (res) {
                    $scope.vehicle_Trip_Grid_Options.dataSource.data(res);
                })
    ...
    $scope.Accept_Click = function ($event, grid, win, vwin) {
        ...
        if ($scope.TripItem.ListORDConName.length == 0 || $scope.TripItem.VehicleID < 1 || $scope.TripItem.RomoocID < 1 || $scope.TripItem.ETA == null || $scope.TripItem.ETD == null) {
            ...
            if ($scope.IsVehicleVendor) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _OPSAppointment_COViewOnMap.URL.DriverVendor_List,
                    data: {
                        vendorID: $scope.VehicleVendorID
                    },
                    success: function (res) {
                ...
                $scope.TripItem.LocationStartID = _OPSAppointment_COViewOnMap.Data.Location.LocationStartID;
                $scope.TripItem.LocationStartName = _OPSAppointment_COViewOnMap.Data.Location.LocationStartName;
                $scope.TripItem.LocationEndID = _OPSAppointment_COViewOnMap.Data.Location.LocationEndID;
                $scope.TripItem.LocationEndName = _OPSAppointment_COViewOnMap.Data.Location.LocationEndName;

                $scope.TripItem.LocationEndLat = _OPSAppointment_COViewOnMap.Data.Location.LocationEndLat;
                $scope.TripItem.LocationEndLng = _OPSAppointment_COViewOnMap.Data.Location.LocationEndLng;
                $scope.TripItem.LocationStartLat = _OPSAppointment_COViewOnMap.Data.Location.LocationStartLat;
                $scope.TripItem.LocationStartLng = _OPSAppointment_COViewOnMap.Data.Location.LocationStartLng;

                $scope.TripItem.LocationGetRomoocID = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocID;
                $scope.TripItem.LocationGetRomoocName = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocName;
                $scope.TripItem.LocationGetRomoocLat = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLat;
                $scope.TripItem.LocationGetRomoocLng = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLng;

                if ($scope.RomoocMustReturn == true) {
                    $scope.TripItem.LocationReturnRomoocID = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocID;
                    $scope.TripItem.LocationReturnRomoocName = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocName;
                    $scope.TripItem.LocationReturnRomoocLat = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLat;
                    $scope.TripItem.LocationReturnRomoocLng = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLng;
                }
                //Kiểm tra đầu kéo, romooc vào khoảng ETD và ETA && Lấy thông tin điểm lấy romooc.
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _OPSAppointment_COViewOnMap.URL.CheckVehicleAvailable,
                    data: {
        ...
        if (flag) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.Save,
                        data: {
                            item: $scope.TripItem
                        },
                        success: function (res) {
        ...
        change: function (e) {
            ...
            $timeout(function () {
                try {
                    var obj = cbo.dataItem(cbo.select());
                    if (Common.HasValue(obj)) {
                        openMapV2.Center(obj.Lat, obj.Lng, 20);
                    }
                } catch (e) {
                }
            }, 10)
        ...
        if ($scope.IsHub) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.Split,
                        data: {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.OPS,
        method: _OPSAppointment_COViewOnMap.URL.Driver_List,
        success: function (res) {
            var data = [];
            $.each(res, function (i, v) {
                data.push({ 'ID': v.ID, 'Text': v.LastName + ' ' + v.FirstName + ' (' + v.EmployeeCode + ')', 'DriverName': v.LastName + ' ' + v.FirstName, 'DriverTel': v.Cellphone });
        ...
        if (flag) {
            $rootScope.Message({
                ...
                Ok: function () {
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.Split_Cancel,
                        data: {
                            conID: item.ParentID
                        },
                        success: function (res) {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.OPS,
        method: _OPSAppointment_COViewOnMap.URL.Vendor_List,
        success: function (res) {
            _OPSAppointment_COViewOnMap.Data.VendorList = res;
    ...
    $scope.Tender_Click = function ($event, grid, win) {
        ...
        if (data.length > 0 && $scope.ViewOrder) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _OPSAppointment_COViewOnMap.URL.Vendor_WithKPI_List,
                data: { data: data },
                success: function (res) {
                    Common.Services.Error(res, function (res) {
        ...
        if (Common.HasValue(data) && data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.ToVendorKPI,
        ...
        if (data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.Cancel,
                        data: {
                            data: data
                        },
                        success: function (res) {
    ...
    $scope.Tender_Suggest_Click = function ($event, grid, win) {
        ...
        if (data.length > 0 && $scope.ViewOrder) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _OPSAppointment_COViewOnMap.URL.TenderKPI_List,
                data: { data: data },
                success: function (res) {
                    Common.Services.Error(res, function (res) {
        ...
        if (Common.HasValue(item)) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.TenderKPI_Save,
    ...
    $scope.new_trip_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.COTOContainer_List,
            pageSize: 100,
    ...
    $scope.new_container_by_trip_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.COTOContainerByTrip_List,
            pageSize: 0,
            readparam: function () {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.ToMon,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.ToOPS,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.Delete,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMap.URL.CO2View_Master_Update_Driver,
                        data: { data: data },
                        success: function (res) {
                            Common.Services.Error(res, function (res) {
                                $rootScope.IsLoading = false;
                    ...
                    case 1:
                        $rootScope.Message({
                            ...
                            Ok: function () {
                                ...
                                Common.Services.Call($http, {
                                    url: Common.Services.url.OPS,
                                    method: _OPSAppointment_COViewOnMap.URL.CO2View_Master_Update_TimeLine,
                ...
                case 1:
                    $rootScope.Message({
                        ...
                        Ok: function () {
                            ...
                            Common.Services.Call($http, {
                                url: Common.Services.url.OPS,
                                method: _OPSAppointment_COViewOnMap.URL.CO2View_Master_ChangeVehicle,
    ...
    $scope.new2view_con_Grid_Options = {
        ...
        filterable: { mode: 'row', visible: false }, reorderable: false, selectable: true, resizable: true, autoBind: false,
        columns: [
            {
    ...
    $scope.new2view_trip_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.CO2View_Container_List,
                ...
                if (!flag || toMasterID < 1) {
                    $rootScope.Message({
                        ...
                        Ok: function () {
                            ...
                            $scope.NewTripItem = {
                                ...
                                ListORDConName: [],
                                ListOPSCon: [],
                                LocationStartID: _OPSAppointment_COViewOnMap.Data.Location.LocationStartID,
                                LocationStartName: _OPSAppointment_COViewOnMap.Data.Location.LocationStartName,
                                LocationEndID: _OPSAppointment_COViewOnMap.Data.Location.LocationEndID,
                                LocationEndName: _OPSAppointment_COViewOnMap.Data.Location.LocationEndName,
                                LocationStartLat: _OPSAppointment_COViewOnMap.Data.Location.LocationStartLat,
                                LocationStartLng: _OPSAppointment_COViewOnMap.Data.Location.LocationStartLng,
                                LocationEndLat: _OPSAppointment_COViewOnMap.Data.Location.LocationEndLat,
                                LocationEndLng: _OPSAppointment_COViewOnMap.Data.Location.LocationEndLng,
                                DateGetRomooc: dataItem.ETD,
                                DateReturnRomooc: null,
                                LocationGetRomoocID: _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocID,
                                LocationGetRomoocName: _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocName,
                                LocationGetRomoocLat: _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLat,
                                LocationGetRomoocLng: _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLng,
                                LocationReturnRomoocID: -1,
                                LocationReturnRomoocName: "",
                                LocationReturnRomoocLat: null,
                                LocationReturnRomoocLng: null,
                            ...
                            if ($scope.RomoocMustReturn == true) {
                                $scope.NewTripItem.LocationReturnRomoocID = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocID;
                                $scope.NewTripItem.LocationReturnRomoocName = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocName;
                                $scope.NewTripItem.LocationReturnRomoocLat = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLat;
                                $scope.NewTripItem.LocationReturnRomoocLng = _OPSAppointment_COViewOnMap.Data.Location.LocationRomoocLng;
                            }
                    ...
                    $rootScope.Message({
                        ...
                        Ok: function () {
                            ...
                            Common.Services.Call($http, {
                                url: Common.Services.url.OPS,
                                method: _OPSAppointment_COViewOnMap.URL.CO2View_Master_Update_Container,
                                data: {
    ...
    $scope.NewTOMasterContainerDelete_Click = function ($event, item, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.CO2View_Master_Update_Check4Delete,
    ...
    $scope.NewTOMasterInfo_Click = function ($event, grid, win) {
        ...
        if (Common.HasValue(obj)) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _OPSAppointment_COViewOnMap.URL.TripByID,
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    if ($scope.NewTripItem.ID < 1) {
                        ...
                        Common.Services.Call($http, {
                            url: Common.Services.url.OPS,
                            method: _OPSAppointment_COViewOnMap.URL.CO2View_Master_Update,
                            data: {
                                item: $scope.NewTripItem
                            },
                            success: function (res) {
    ...
    $scope.LoadNewTimeLineData = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMap.URL.Schedule_Data,
        ...
        Common.Services.Call($http, {
            ...
            success: function (res) {
                Common.Services.Error(res, function (res) {
                    ...
                    $rootScope.Message({ Msg: "Vui lòng tải lại trang để thiết lập có hiệu lực!", Type: Common.Message.Type.Alert });
                })
                $rootScope.IsLoading = false;
            }
        })
