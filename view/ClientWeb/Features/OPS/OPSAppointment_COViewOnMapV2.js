...
var _OPSAppointment_COViewOnMapV2 = {
    URL: {
...
angular.module('myapp').controller('OPSAppointment_COViewOnMapV2Ctrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', '$compile', 'openMapV2', function ($rootScope, $scope, $http, $location, $state, $timeout, $compile, openMapV2) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COViewOnMapV2Ctrl');
    ...
    //#region Layout
    $scope.ViewVertical = false;
    var objCookie = Common.Cookie.Get("COViewOnMapV2_View");
    if (objCookie != $state.params.view) {
        $state.go("main.OPSAppointment.COViewOnMapV2", { view: objCookie });
    }
    else if (objCookie == 'ver') {
        $scope.ViewVertical = true;
    }

    $scope.viewSplitter_Options = {
        orientation: $scope.ViewVertical ? "vertical" : "horizontal",
        panes: [
            { collapsible: true, resizable: true },
            { collapsible: true, resizable: true, size: $scope.ViewVertical ? '300px' : '500px', min: $scope.ViewVertical ? '200px' : '480px' }
        ],
        resize: function (e) {
            try {
                openMapV2.Resize();
    ...
    $scope.conSplitter_Options = {
        orientation: $scope.ViewVertical ? "horizontal" : "vertical",
        panes: [
            { collapsible: false, resizable: false, size: '100px', min: '100px' },
    ...
    $timeout(function () {
        //Chuyển div chuyến - xóa pane 1
        if ($scope.ViewVertical) {
            $container = angular.copy(angular.element('#trip-view'));
            $container.addClass('on-map-view');
            $('#trip-view-container').append($compile($container)($scope));
            $scope.conSplitter.remove('.con-splitter #trip-view');
            if (!$scope.ConfigView.showMap) {
                $($scope.viewSplitter.element).find('.k-splitbar.k-splitbar-vertical').hide();
                $scope.viewSplitter.size(".k-pane:first", "106px");
            }
        } else {
            var pane = $($scope.viewSplitter.element).find(".k-pane:first");
            if (!$scope.ConfigView.showMap) {
    ...
    $scope.SwitchView_Click = function ($event) {
        ...
        try {
            ...
            if (view == 'ver') {
                Common.Cookie.Clear("COViewOnMapV2_View");
                $state.go("main.OPSAppointment.COViewOnMapV2", { view: undefined });
            } else {
                Common.Cookie.Set("COViewOnMapV2_View", 'ver');
                $state.go("main.OPSAppointment.COViewOnMapV2", { view: 'ver' });
            }
        } catch (e) {
        }
    }
    ...
    Common.Services.Call($http, {
        ...
        method: _OPSAppointment_COViewOnMapV2.URL.Setting,
        success: function (res) {
            if (Common.HasValue(res)) {
                ...
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartID = res.LocationStartID;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartName = res.LocationStartName;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndID = res.LocationEndID;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndName = res.LocationEndName;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndLat = res.LocationEndLat;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndLng = res.LocationEndLng;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartLat = res.LocationStartLat;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartLng = res.LocationStartLng;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocID = res.LocationRomoocGetID;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocName = res.LocationRomoocGetName;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLat = res.LocationRomoocGetLat;
                _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLng = res.LocationRomoocGetLng;
            }
        }
    });
    ...
    $scope.con_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.Order_List,
            pageSize: 20,
            readparam: function () {
                return {
        ...
        height: '99%', groupable: true, sortable: true, columnMenu: false, resizable: true, filterable: { mode: 'row', visible: false }, reorderable: false, autoBind: false,
    ...
    $scope.romooc_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.Romooc_List,
            readparam: function () {
                return {
    ...
    $scope.tractor_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.Tractor_List,
            readparam: function () {
                return {
                    requestDate: $scope.VehicleRequestDate
                }
    ...
    $scope.location_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.Location_List,
            pageSize: 20,
            model: {
                id: 'ID',
                fields: {
    ...
    $scope.ViewVehicleInfo = function ($event, item, type) {
        ...
        if ($scope.VehicleInfoStyle.left < 0)
            $scope.VehicleInfoStyle.left = off.left + 100;

        if (!Common.HasValue(_OPSAppointment_COViewOnMapV2.Data.VehicleInfo[type + "_" + item.ID])) {
            _OPSAppointment_COViewOnMapV2.Data.VehicleInfo[type + "_" + item.ID] = {
                IsLoaded: false,
                ListTrip: []
            }
            ...
            $scope.LoadVehicleTrip(new Date(), type == 1 ? item.ID : -1, type == 2 ? item.ID : -1, 3, true, true, function (res) {
                ...
                var obj = _OPSAppointment_COViewOnMapV2.Data.VehicleInfo[type + "_" + item.ID];
                $scope.VehicleInfo = {
    ...
    $scope.LoadVehicleTrip = function (requestDate, vehicleID, romoocID, total, isapproved, istendered, callback) {
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.TripByVehicle_List,
            data: {
    ...
    $scope.Tractor_Select = function ($event, item, grid) {
        ...
        if (Common.HasValue(item)) {
            ...
            if (item.RomoocID > 0) {
                $scope.TripItem.RomoocID = item.RomoocID;
                $scope.TripItem.RomoocNo = item.RomoocNo;
            }
        }
    }
    ...
    $scope.Accept_Click = function ($event, grid, win, vwin) {
        ...
        if ($scope.TripItem.RomoocID == null || $scope.TripItem.RomoocID == "") {
            $scope.TripItem.RomoocID = -1;
        }
        if ($scope.TripItem.ListORDConName.length == 0) {
            $rootScope.Message({ Msg: "Vui lòng chọn đơn hàng.", Type: Common.Message.Type.Alert });
        } else if ($scope.TripItem.VehicleID < 1) {
            ...
            $rootScope.Message({ Msg: "Vui lòng nhập thời gian ETD và ETA.", Type: Common.Message.Type.Alert });
        } else {
            $rootScope.IsLoading = true;
            $scope.TripItem.ListORDConName = dataCode;
            if ($scope.ViewOrder) {
            ...
            if ($scope.IsVehicleVendor) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _OPSAppointment_COViewOnMapV2.URL.DriverVendor_List,
                    data: {
                        vendorID: $scope.VehicleVendorID
                    },
                    success: function (res) {
                ...
                $scope.TripItem.LocationStartID = _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartID;
                $scope.TripItem.LocationStartName = _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartName;
                $scope.TripItem.LocationStartLat = _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartLat;
                $scope.TripItem.LocationStartLng = _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartLng;

                $scope.TripItem.LocationEndID = _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndID;
                $scope.TripItem.LocationEndName = _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndName;
                $scope.TripItem.LocationEndLat = _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndLat;
                $scope.TripItem.LocationEndLng = _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndLng;

                $scope.TripItem.LocationGetRomoocID = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocID;
                $scope.TripItem.LocationGetRomoocName = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocName;
                $scope.TripItem.LocationGetRomoocLat = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLat;
                $scope.TripItem.LocationGetRomoocLng = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLng;

                if ($scope.RomoocMustReturn == true) {
                    $scope.TripItem.LocationReturnRomoocID = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocID;
                    $scope.TripItem.LocationReturnRomoocName = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocName;
                    $scope.TripItem.LocationReturnRomoocLat = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLat;
                    $scope.TripItem.LocationReturnRomoocLng = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLng;
                }

                //Kiểm tra đầu kéo, romooc vào khoảng ETD và ETA && Lấy thông tin điểm lấy romooc.            
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _OPSAppointment_COViewOnMapV2.URL.CheckVehicleAvailable,
                    data: {
    ...
    $scope.Save_Click = function ($event, win) {
        ...
        if (flag) {
            var msg = "Xác nhận lưu?";
            if ($scope.TripItem.DriverName == "" || $scope.TripItem.DriverName == null) {
                msg = "Không có thông tin tài xế, xác nhận lưu?";
            }
            $rootScope.Message({
                Msg: msg, Type: Common.Message.Type.Confirm,
                Ok: function () {
                    $rootScope.IsLoading = true;
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.Save,
                        data: {
                            item: $scope.TripItem
                        },
                        success: function (res) {
    ...
    $scope.Location_Select = function ($event, item, win) {
        ...
        if ($scope.NewTimeLineDetail) {
            switch ($scope.LocationType) {
                case 1: //Start
                    $scope.NewTimeLineItem.LocationStartID = item.ID;
                    $scope.NewTimeLineItem.LocationStartName = item.Location;
                    $scope.NewTimeLineItem.LocationStartLat = item.Lat;
                    $scope.NewTimeLineItem.LocationStartLng = item.Lng;
                    break;
                case 2: //RomoocGet
                    $scope.NewTimeLineItem.LocationGetRomoocID = item.ID;
                    $scope.NewTimeLineItem.LocationGetRomoocName = item.Location;
                    $scope.NewTimeLineItem.LocationGetRomoocLat = item.Lat;
                    $scope.NewTimeLineItem.LocationGetRomoocLng = item.Lng;
                    break;
                case 3: //RomoocReturn
                    $scope.NewTimeLineItem.LocationReturnRomoocID = item.ID;
                    $scope.NewTimeLineItem.LocationReturnRomoocName = item.Location;
                    $scope.NewTimeLineItem.LocationReturnRomoocLat = item.Lat;
                    $scope.NewTimeLineItem.LocationReturnRomoocLng = item.Lng;
                    break;
                case 4: //End
                    $scope.NewTimeLineItem.LocationEndID = item.ID;
                    $scope.NewTimeLineItem.LocationEndName = item.Location;
                    $scope.NewTimeLineItem.LocationEndLat = item.Lat;
                    $scope.NewTimeLineItem.LocationEndLng = item.Lng;
                    break;
            }
        }
        else if ($scope.IsShowNewTrip) {
            switch ($scope.LocationType) {
                case 1: //Start
                    ...
                    break;
                case 2: //RomoocGet
                    $scope.NewTripItem.LocationGetRomoocID = item.ID;
                    $scope.NewTripItem.LocationGetRomoocName = item.Location;
                    $scope.NewTripItem.LocationGetRomoocLat = item.Lat;
                    $scope.NewTripItem.LocationGetRomoocLng = item.Lng;
                    break;
                case 3: //RomoocReturn
                    $scope.NewTripItem.LocationReturnRomoocID = item.ID;
                    $scope.NewTripItem.LocationReturnRomoocName = item.Location;
                    $scope.NewTripItem.LocationReturnRomoocLat = item.Lat;
                    $scope.NewTripItem.LocationReturnRomoocLng = item.Lng;
                    break;
                case 4: //End
        ...
        if ($scope.IsHub) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.Split,
                        data: {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.OPS,
        method: _OPSAppointment_COViewOnMapV2.URL.Driver_List,
        success: function (res) {
            var data = [];
            $.each(res, function (i, v) {
                data.push({ 'ID': v.ID, 'Text': v.LastName + ' ' + v.FirstName + ' (' + v.EmployeeCode + ')', 'DriverName': v.LastName + ' ' + v.FirstName, 'DriverTel': v.Cellphone });
    ...
    $scope.CheckTrip = function (props) {
        $scope.TripItem.StatusCode = "";
        $scope.TripItem.StatusColor = $scope.Color.None;

        if (props == "ETD" && $scope.TripItem.ETD != null && $scope.TripItem.ETA != null) {
            if ($scope.TripItem.ETD >= $scope.TripItem.ETA || $scope.TripItem.ETD.addDays(1 / 48) > $scope.TripItem.ETA) {
                $scope.TripItem.ETA = $scope.TripItem.ETD.addDays(1 / 48);
            }
        }
                                ...
                                if ($scope.TripItem.DataContainerOffer == null || $scope.TripItem.DataContainerOffer.length == 0) {
                                    ...
                                    if (total > res.MaxCapacity) {
                                        ...
                                        if (res.IsVehicleAvailable && res.IsRomoocAvailable) {
                                            ...
                                            $scope.TripItem.StatusColor = $scope.Color.Warning;
                                        }
                                    }
                                }
                            }
        ...
        if (flag) {
            $rootScope.Message({
                ...
                Ok: function () {
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.Split_Cancel,
                        data: {
                            conID: item.ParentID
                        },
                        success: function (res) {
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.OPS,
        method: _OPSAppointment_COViewOnMapV2.URL.Vendor_List,
        success: function (res) {
            _OPSAppointment_COViewOnMapV2.Data.VendorList = res;
    ...
    $scope.Tender_Click = function ($event, grid, win) {
        ...
        if (data.length > 0 && $scope.ViewOrder) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _OPSAppointment_COViewOnMapV2.URL.Vendor_WithKPI_List,
                data: { data: data },
                success: function (res) {
                    Common.Services.Error(res, function (res) {
        ...
        if (Common.HasValue(data) && data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.ToVendorKPI,
        ...
        if (data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.Cancel,
                        data: {
                            data: data
                        },
                        success: function (res) {
    ...
    $scope.vendor_rate_Grid_Options = {
        ...
        height: '99%', groupable: false, pageable: Common.PageSize, columnMenu: false, resizable: false, reorderable: false, sortable: false, autoBind: false,
        dataBound: function (e) {
            var grid = this;
    ...
    $scope.Tender_Suggest_Click = function ($event, grid, win) {
        ...
        if (data.length > 0 && $scope.ViewOrder) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _OPSAppointment_COViewOnMapV2.URL.TenderKPI_List,
                data: { data: data },
                success: function (res) {
                    Common.Services.Error(res, function (res) {
    ...
    $scope.vendor_finance_Grid_Options = {
        ...
        height: '99%', groupable: false, pageable: Common.PageSize, columnMenu: false, resizable: true, reorderable: false, sortable: true, selectable: true, autoBind: false,
        columns: [
        ...
        if (Common.HasValue(item)) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.TenderKPI_Save,
    ...
    $scope.new_trip_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.COTOContainer_List,
            pageSize: 100,
        ...
        height: '99%', groupable: false, pageable: Common.PageSize, columnMenu: false, resizable: true, reorderable: false, sortable: true, filterable: { mode: 'row' }, autoBind: false,
        dataBound: function () {
            $scope.NewTripChoose = false;
    ...
    $scope.new_container_by_trip_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.COTOContainerByTrip_List,
            pageSize: 0,
            readparam: function () {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.ToMon,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.ToOPS,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.Delete,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        if ($scope.NewTripViewAction == 0) {
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Master_Update_Driver,
                        data: { data: data },
                        success: function (res) {
                            Common.Services.Error(res, function (res) {
                                $rootScope.IsLoading = false;
                    ...
                    case 1:
                        $rootScope.Message({
                            ...
                            Ok: function () {
                                ...
                                Common.Services.Call($http, {
                                    url: Common.Services.url.OPS,
                                    method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Master_Update_TimeLine,
                ...
                case 1:
                    $rootScope.Message({
                        ...
                        Ok: function () {
                            ...
                            Common.Services.Call($http, {
                                url: Common.Services.url.OPS,
                                method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Master_ChangeVehicle,
    ...
    $scope.new2view_trip_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Container_List,
        ...
        reorderable: false, sortable: true, filterable: { mode: 'row' }, selectable: true, autoBind: false,
        change: function (e) {
            var grid = this;
                ...
                if (!flag || toMasterID < 1) {
                    $rootScope.Message({
                        ...
                        Ok: function () {
                            ...
                            $scope.NewTripItem = {
                                ...
                                ListORDConName: [],
                                ListOPSCon: [],
                                LocationStartID: _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartID,
                                LocationStartName: _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartName,
                                LocationEndID: _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndID,
                                LocationEndName: _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndName,
                                LocationStartLat: _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartLat,
                                LocationStartLng: _OPSAppointment_COViewOnMapV2.Data.Location.LocationStartLng,
                                LocationEndLat: _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndLat,
                                LocationEndLng: _OPSAppointment_COViewOnMapV2.Data.Location.LocationEndLng,
                                DateGetRomooc: dataItem.ETD,
                                DateReturnRomooc: null,
                                LocationGetRomoocID: _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocID,
                                LocationGetRomoocName: _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocName,
                                LocationGetRomoocLat: _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLat,
                                LocationGetRomoocLng: _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLng,
                                LocationReturnRomoocID: -1,
                                LocationReturnRomoocName: "",
                                LocationReturnRomoocLat: null,
                                LocationReturnRomoocLng: null,
                            ...
                            if ($scope.RomoocMustReturn == true) {
                                $scope.NewTripItem.LocationReturnRomoocID = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocID;
                                $scope.NewTripItem.LocationReturnRomoocName = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocName;
                                $scope.NewTripItem.LocationReturnRomoocLat = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLat;
                                $scope.NewTripItem.LocationReturnRomoocLng = _OPSAppointment_COViewOnMapV2.Data.Location.LocationRomoocLng;
                            }
                    ...
                    $rootScope.Message({
                        ...
                        Ok: function () {
                            ...
                            Common.Services.Call($http, {
                                url: Common.Services.url.OPS,
                                method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Master_Update_Container,
                                data: {
    ...
    $scope.NewTOMasterContainerDelete_Click = function ($event, item, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Master_Update_Check4Delete,
    ...
    $scope.NewTOMasterInfo_Click = function ($event, grid, win) {
        ...
        if (Common.HasValue(obj)) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _OPSAppointment_COViewOnMapV2.URL.TripByID,
            ...
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    if ($scope.NewTripItem.ID < 1) {
                        ...
                        Common.Services.Call($http, {
                            url: Common.Services.url.OPS,
                            method: _OPSAppointment_COViewOnMapV2.URL.CO2View_Master_Update,
                            data: {
                                item: $scope.NewTripItem
                            },
                            success: function (res) {
    ...
    $scope.LoadNewTimeLineData = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _OPSAppointment_COViewOnMapV2.URL.Schedule_Data,
    ...
    $scope.ConfigView_ShowMap_Click = function ($event) {
        ...
        if ($scope.ConfigView.showMap) {
            if ($scope.ViewVertical) {
                $($scope.viewSplitter.element).find('.k-splitbar.k-splitbar-vertical').show();
                $scope.viewSplitter.size(".k-pane:first", "300px");
            } else {
                $($scope.viewSplitter.element).find('.k-splitbar:eq(0)').show();
                $scope.viewSplitter.expand(pane);
            }
        } else {
            if ($scope.ViewVertical) {
                $($scope.viewSplitter.element).find('.k-splitbar.k-splitbar-vertical').hide();
    ...
    $scope.$watch("InitComplete", function () {
