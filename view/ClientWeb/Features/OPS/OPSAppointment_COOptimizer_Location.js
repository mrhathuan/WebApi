...
angular.module('myapp').controller('OPSAppointment_COOptimizer_LocationCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COOptimizer_LocationCtrl');
        ...
        if (Common.HasValue(objCookie)) {
            if (objCookie.ID != $scope.OptimizerID) {
                Common.Services.Call($http, {
                    ...
                    method: _COOptimizer_Location.URL.Optimizer_Get,
                    data: {
    ...
    $scope.locationGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Location.URL.Location_List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
    ...
    $scope.time_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Location.URL.Location_Require_List,
            readparam: function () {
                return {
    ...
    $scope.cboDayOfWeek_Options = {
        ...
        dataSource: Common.DataSource.Local({
            data: _COOptimizer_Location.Data.DayOfWeek,
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number' },
    ...
    $scope.New_Time_Click = function ($event, grid) {
        ...
        grid.dataSource.insert(0, $.extend(true, {}, _COOptimizer_Location.Data.ItemNew));
        $timeout(function () {
    ...
    $scope.Time_Delete_Click = function ($event, item, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Location.URL.Location_Require_Remove,
            data: {
                data: [item.ID]
            },
            success: function (res) {
    ...
    $scope.Time_Save_Click = function ($event, item, grid) {
        ...
        if ($scope.TimeSaved == false) {
            if (item.TimeFrom != null && item.TimeTo != null && item.DayOfWeek != null) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer_Location.URL.Location_Require_Save,
                    data: {
                        optLocationID: $scope.optLocationID,
        ...
        if (data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _COOptimizer_Location.URL.Location_Require_Reset,
                        data: {
                            data: data
                        },
                        success: function (res) {
