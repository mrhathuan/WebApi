...
angular.module('myapp').controller('OPSAppointment_COOptimizerCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COOptimizerCtrl');
    $rootScope.IsLoading = false;

    $scope.HasSelect = false;
    $scope.Optimizer = { ID: -1, IsContainer: true, OptimizerName: '', DateFrom: new Date().addDays(-3), DateTo: new Date() };

    $scope.optimizerGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer.URL.List,
            readparam: function () {
                return {
                    isCo: true
                }
            ...
            model: {
                ...
                fields: {
                    ...
                    DateFrom: { type: 'date' },
                    DateTo: { type: 'date' },
                    IsBalanceCustomer: { type: 'bool' },
                    IsBalanceKMScore: { type: 'bool' }
                }
            },
            pageSize: 20
        }),
        ...
        columns: [
            ...
            {
                field: 'OptimizerName',width: '200px', title: 'Tên optimizer', filterable: { cell: { operator: 'contains', showOperators: false } },
            ...
            {
                field: 'StatusOfOptimizerName', width: '150px', title: 'Trạng thái', filterable: { cell: { operator: 'contains', showOperators: false } },
                sortorder: 4, configurable: true, isfunctionalHidden: false
            },
            {
                field: 'IsBalanceKMScore', width: '120px', title: 'Cân bằng cự ly', template: "<input type='checkbox' disabled='true' ng-model='dataItem.IsBalanceKMScore' />", filterable: false,
                sortorder: 5, configurable: true, isfunctionalHidden: false
            },
            {
                field: 'IsBalanceCustomer', width: '150px', title: 'Cân bằng số chuyến', template: "<input type='checkbox' disabled='true' ng-model='dataItem.IsBalanceKMScore' />", filterable: false ,
                sortorder: 6, configurable: true, isfunctionalHidden: false
            },
            {
                field: 'CreatedBy', width: '200px', title: 'Người tạo', filterable: { cell: { operator: 'contains', showOperators: false } },
        ...
        dataBound: function () {
            this.element.find('tr').dblclick(function (e) {
                ...
                if (Common.HasValue(item)) {
                    ...
                    Common.Cookie.Set("OPSCOOptimizer", JSON.stringify(item));
                    $state.go("main.OPSAppointment.COOptimizer_Container", { OptimizerID: item.ID });
                }
            });
        }
    }
    ...
    $scope.Optimizer_Delete = function ($event, item, grid) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer.URL.Delete,
