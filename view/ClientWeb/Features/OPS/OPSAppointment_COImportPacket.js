...
var _COImportPacket = {
    URL: {
        List: 'OPS_CO_Import_Packet_List',
        Save: 'OPS_CO_Import_Packet_Save',
        Delete: 'OPS_CO_Import_Packet_Delete',
        SettingPlan_List: 'OPS_CO_Import_Packet_SettingPlan',
    }
}

angular.module('myapp').controller('OPSAppointment_COImportPacketCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COImportPacketCtrl');
    ...
    $scope.packetGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            ...
            method: _COImportPacket.URL.List,
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number' },
                    TotalTon: { type: 'number' },
                    TotalContainer: { type: 'number' },
                    IsCreated: { type: 'bool' }
                }
            },
            readparam: function () {
        ...
        columns: [
            ...
            {
                field: 'TotalTon', width: '100px', title: 'Tấn',
                filterable: { cell: { operator: 'gte', showOperators: false } }
            },
            {
                field: 'TotalContainer', title: 'SL cont',
                filterable: { cell: { operator: 'gte', showOperators: false } }
            }
        ],
        ...
        dataBound: function () {
            this.element.find('tr').dblclick(function (e) {
                ...
                if (Common.HasValue(item)) {
                    $state.go("main.OPSAppointment.COImportPacket_Detail_Input", { ID: item.ID });
                }
            });
        }
    }
    ...
    Common.Services.Call($http, {
        url: Common.Services.url.OPS,
        method: _COImportPacket.URL.SettingPlan_List,
    ...
    $scope.Packet_Delete = function ($event, item, grid) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COImportPacket.URL.Delete,
                    data: { pID: item.ID },
    ...
    $scope.Packet_Save_Click = function ($event, grid, win) {
        ...
        if ($scope.PacketItem.PacketName == '' || $scope.PacketItem.PacketName == null) {
            ...
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _COImportPacket.URL.Save,
                ...
                success: function (res) {
                    Common.Services.Error(res, function () {
                        ...
                        win.close();
                        $state.go("main.OPSAppointment.COImportPacket_Detail_Input", { ID: res });
                    })
                }
            })
        }
