...
angular.module('myapp').controller('OPSAppointment_COOptimizer_RouteCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', 'openMap', function ($rootScope, $scope, $http, $location, $state, $timeout, openMap) {
    Common.Log('OPSAppointment_COOptimizer_RouteCtrl');
        ...
        if (Common.HasValue(objCookie)) {
            if (objCookie.ID != $scope.OptimizerID) {
                Common.Services.Call($http, {
                    ...
                    method: _COOptimizer_Route.URL.Optimizer_Get,
                    data: {
    ...
    $scope.routeGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Route.URL.Routing_List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
    ...
    $scope.time_Grid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Route.URL.Routing_Require_List,
            readparam: function () {
                return {
    ...
    $scope.cboDayOfWeek_Options = {
        ...
        dataSource: Common.DataSource.Local({
            data: _COOptimizer_Route.Data.DayOfWeek,
            model: {
                id: 'ID',
                fields: {
                    ID: { type: 'number' },
    ...
    $scope.New_Time_Click = function ($event, grid) {
        ...
        grid.dataSource.insert(0, $.extend(true, {}, _COOptimizer_Route.Data.ItemNew));
        $timeout(function () {
    ...
    $scope.Time_Delete_Click = function ($event, item, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Route.URL.Routing_Require_Remove,
            data: {
                data: [item.ID]
            },
            success: function (res) {
    ...
    $scope.Time_Save_Click = function ($event, item, grid) {
        ...
        if ($scope.TimeSaved == false) {
            if (item.TimeFrom != null && item.TimeTo != null && item.DayOfWeek != null) {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer_Route.URL.Routing_Require_Save,
                    data: {
        ...
        if (data.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _COOptimizer_Route.URL.Routing_Require_Reset,
                        data: {
                            data: data
                        },
                        success: function (res) {
        ...
        angular.forEach(data, function (o) {
            try {
                openMap.Distance({ Lat: o.FromLat, Lng: o.FromLng }, { Lat: o.ToLat, Lng: o.ToLng }, function (p1, p2, p) {
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _COOptimizer_Route.URL.Opt_Routing_Save,
