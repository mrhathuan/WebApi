...
angular.module('myapp').controller('OPSAppointment_COOptimizer_MasterCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', 'openMapV2', function ($rootScope, $scope, $http, $location, $state, $timeout, openMapV2) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COOptimizer_MasterCtrl');
    ...
    $scope.COTOMaster = { ID: -1, Note: '', ETA: null, ETD: null, VehicleNo: "", RomoocNo: "", VehicleID: "", RomoocID: "" };
    ...
    $scope.master_gridOptions = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Master.URL.List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
            ...
            $timeout(function () {
                ...
                angular.forEach(grid.select(), function (tr) {
                    ...
                    if (Common.HasValue(item)) {
                        var obj = _COOptimizer_Master.Data.MasterLocation[item.ID];
        ...
        columns: [
            {
                ...
                headerTemplate: '<input class="chkSelectAll" type="checkbox" ng-click="gridChooseAll_Check($event,master_grid,coGridChoose_Change)" />',
                headerAttributes: { style: 'text-align: center;' }, attributes: { style: 'text-align: center;' },
                template: '<input class="chkChoose" type="checkbox" #= IsChoose ? "checked=checked" : "" # ng-click="gridChoose_Check($event,master_grid,coGridChoose_Change)" />',
            ...
            {
                field: 'VendorOfVehicleName', width: '150px', title: 'Đối tác',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
            ...
            {
                field: 'RomoocNo', width: '100px', title: 'Số romooc',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
            ...
            {
                field: 'KM', width: '100px', title: 'Khoảng cách', template: '',
    ...
    $scope.routingGrid_Options = {
        ...
        columns: [
            {
                field: 'LocationFromName', width: '150px', title: 'Điểm đi',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
                field: 'LocationFromAddress', width: '200px', title: 'Địa chỉ đi',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
            ...
            {
                field: 'LocationToName', width: '150px', title: 'Điểm đến',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
                field: 'LocationToAddress', width: '200px', title: 'Địa chỉ đến',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
    ...
    $scope.Detail_Click = function ($event, item, win) {
        ...
        $scope.COTOMaster = {
            ...
            VehicleNo: item.VehicleNo, RomoocNo: item.RomoocNo, VehicleID: item.VehicleID, RomoocID: item.RomoocID
        }
        Common.Log($scope.COTOMaster);
        $scope.containerGrid_Options.dataSource.read();
        win.center().open();
    }

    $scope.containerGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Master.URL.Master_Container_List,
            readparam: function () {
                return {
                    optMasterID: $scope.COTOMaster.ID
                }
            },
            pageSize: 0,
            model: {
    ...
    $scope.LoadData = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Master.URL.Location_Data,
            data: {
                optimizerID: $scope.OptimizerID
            },
            success: function (res) {
                angular.forEach(res.Data, function (o, i) {
                    _COOptimizer_Master.Data.MasterLocation[o.ID] = o;
                })
            }
        })
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Master.URL.Vehicle_Data,
            data: {
                optimizerID: $scope.OptimizerID,
                request: ''
            },
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Master.URL.Romooc_Data,
            data: {
                optimizerID: $scope.OptimizerID,
                request: ''
            },
    ...
    $scope.Optimize_ToMon_Click = function ($event) {
        ...
        $rootScope.Message({
            Type: Common.Message.Type.Confirm,
            Msg: 'Xác nhận lưu chuyến?',
            Ok: function () {
                $rootScope.IsLoading = true;

                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer_Master.URL.ToMon,
    ...
    $scope.Accept_Click = function ($event, win) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer_Master.URL.Change,
                    data: { item: $scope.COTOMaster },
                    success: function (res) {
                        Common.Services.Error(res, function (res) {
                            $rootScope.IsLoading = false;
                            $rootScope.Message({ Msg: "Đã cập nhật!" });
    ...
    $scope.Delete_Click = function ($event, win) {
        ...
        $rootScope.Message({
            ...
            Ok: function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer_Master.URL.Delete,
                    data: { optMasterID: $scope.COTOMaster.ID },
                    success: function (res) {
                        Common.Services.Error(res, function (res) {
                            $rootScope.IsLoading = false;
    ...
    $scope.Run = function () {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Master.URL.Location_Matrix_List,
            ...
            success: function (res) {
                Common.Services.Error(res, function (res) {
                    if (res == null || res.length == 0) {
                        ...
                        Common.Services.Call($http, {
                            url: Common.Services.url.OPS,
                            method: _COOptimizer_Master.URL.Run,
                            data: { optimizerID: $scope.OptimizerID },
                            success: function (res) {
                                Common.Services.Error(res, function (res) {
                                    ...
                                    Common.Services.Call($http, {
                                        url: Common.Services.url.OPS,
                                        method: _COOptimizer_Master.URL.Cal,
                                        data: { optimizerID: $scope.OptimizerID },
                                        success: function (res) {
                                            Common.Services.Error(res, function (res) {
                                                ...
                                                Common.Services.Call($http, {
                                                    url: Common.Services.url.OPS,
                                                    method: _COOptimizer_Master.URL.Out,
                                                    data: { optimizerID: $scope.OptimizerID },
                                                    success: function (res) {
                                                        Common.Services.Error(res, function (res) {
    ...
    $scope.Error = function () {
        ...
        $scope.Notification_Text = "Lỗi. Optimize không thành công! Quay lại trang đơn hàng";
        $timeout(function () {
            $scope.notification_win.close();
            $state.go("main.OPSAppointment.COOptimizer_Container", { OptimizerID: $scope.OptimizerID });
        }, 1000)
    }
