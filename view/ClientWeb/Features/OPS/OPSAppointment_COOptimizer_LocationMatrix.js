...
var _COOptimizer_LocationMatrix = {
    URL: {
...
angular.module('myapp').controller('OPSAppointment_COOptimizer_LocationMatrixCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', function ($rootScope, $scope, $http, $location, $state, $timeout) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COOptimizer_LocationMatrixCtrl');
        ...
        if (Common.HasValue(objCookie)) {
            if (objCookie.ID != $scope.OptimizerID) {
                Common.Services.Call($http, {
                    ...
                    method: _COOptimizer_LocationMatrix.URL.Optimizer_Get,
                    data: {
    ...
    $scope.matrixGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_LocationMatrix.URL.Matrix_List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
        ...
        save: function (e) {
            ...
            $timeout(function () {
                ...
                Common.Services.Call($http, {
                    url: Common.Services.url.OPS,
                    method: _COOptimizer_LocationMatrix.URL.Matrix_Update,
                    data: { item: item },
                    success: function (res) {
                        Common.Services.Error(res, function (res) {
                            $rootScope.IsLoading = false;
    ...
    $scope.Refresh_Click = function ($event, grid) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_LocationMatrix.URL.Matrix_Refresh,
