...
angular.module('myapp').controller('OPSAppointment_COOptimizer_VehicleCtrl', ['$rootScope', '$scope', '$http', '$location', '$state', '$timeout', 'openMapV2', function ($rootScope, $scope, $http, $location, $state, $timeout, openMapV2) {
    if ($rootScope.IsPageComplete != true) return;
    Common.Log('OPSAppointment_COOptimizer_VehicleCtrl');
    ...
    $scope.vehicleGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Vehicle.URL.Vehicle_List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
        ...
        height: '100%', pageable: Common.PageSize, sortable: true, columnMenu: false, resizable: true, reorderable: false, filterable: false, selectable: true,
        ...
        columns: [
            ...
            {
                field: 'RomoocNo', title: 'Số romooc', width: '100px',
                headerAttributes: { style: 'text-align: center;' }, attributes: { style: 'text-align: center;' },
                template: "<span ng-show='StatusOfOptimizer == 2'>#=RomoocNo#</span><a ng-show='StatusOfOptimizer < 2' class='k-button' ng-click='Romooc_Change($event,dataItem,romooc_win)'>#=RomoocID > 0 ? RomoocNo : '--- Chọn ---'#</a>",
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
    ...
    $scope.vehicleNotInGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Vehicle.URL.Vehicle_NotIn_List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
        ...
        columns: [
            ...
            {
                field: 'RomoocNo', width: '150px', title: 'Số romooc', filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
        ...
        if (lstID.length > 0) {
            $rootScope.Message({
                ...
                Ok: function () {
                    ...
                    Common.Services.Call($http, {
                        url: Common.Services.url.OPS,
                        method: _COOptimizer_Vehicle.URL.Vehicle_Remove,
    ...
    $scope.Romooc_Change = function ($event, item, win) {
        $event.preventDefault();

        $scope.OptimizerVehicleID = item.ID;
        win.center().open();
        $scope.romoocGrid_Options.dataSource.read();
    };

    $scope.UpdateRomooc = function (romoocID, win) {
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Vehicle.URL.Vehicle_Update,
            data: { vehicleID: $scope.OptimizerVehicleID, romoocID: romoocID },
            success: function (res) {
    ...
    $scope.Vehicle_NotIn_Accept_Click = function ($event, grid, win) {
        ...
        if (data.length > 0) {
            Common.Services.Call($http, {
                url: Common.Services.url.OPS,
                method: _COOptimizer_Vehicle.URL.Vehicle_SaveList,
    ...
    $scope.romoocGrid_Options = {
        dataSource: Common.DataSource.Grid($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Vehicle.URL.Romooc_List,
            readparam: function () {
                return {
                    optimizerID: $scope.OptimizerID
                }
        ...
        height: '100%', pageable: false, sortable: true, columnMenu: false,
        reorderable: false, filterable: false, selectable: true, resizable: true,
        columns: [
            {
                field: 'RomoocNo', title: 'Số Romooc',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            },
            {
                field: 'MaxWeightCal', width: '250px', title: 'Trọng tải',
                filterable: { cell: { operator: 'contains', showOperators: false } }
            }
        ]
    };
    ...
    $scope.Vehicle_Save_Click = function ($event, win) {
        ...
        Common.Services.Call($http, {
            url: Common.Services.url.OPS,
            method: _COOptimizer_Vehicle.URL.Vehicle_UpdateWeight,
    ...
    $scope.Romooc_Cancel_Click = function ($event, win) {
        $event.preventDefault();
        $scope.UpdateRomooc(null, win);
    }

    $scope.Romooc_Accept_Click = function ($event, grid, win) {
        $event.preventDefault();
        ...
        $scope.UpdateRomooc(romoocID, win);
    }
